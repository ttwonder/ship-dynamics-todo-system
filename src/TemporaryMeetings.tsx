...
export default function TemporaryMeetingsPage({ data, visibleVessels, currentUser, canExportReports, setData, commit }: Props) {
  ...
  const filtered = accessibleMeetings.filter(meeting => {
    ...
    return !q || `${meeting.subject} ${richTextToPlainText(meeting.reason)} ${richTextToPlainText(meeting.resolution)} ${[...meeting.participantUserIds, ...meeting.responsibleUserIds].map(id => users[id]?.name || '').join(' ')} ${meetingTaskItems(meeting, data.tasks).map(item => richTextToPlainText(item.description)).join(' ')} ${meeting.meetingDate} ${meetingScopeLabel(meeting)}`.toLowerCase().includes(q);
  });
