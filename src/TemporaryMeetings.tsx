...
import { reconcileMeetingTasks, meetingTaskDescription, meetingTaskNotificationEvents, shouldPreserveMeetingTaskDescriptions } from './meetingTaskWorkflow';
...
type MeetingDraft = Pick<
  TemporaryMeeting,
  'subject' | 'meetingDate' | 'vessels' | 'reason' | 'departments' | 'resolution' | 'taskDescription' | 'expectedDate' | 'priority'
> & {
...
const draftFrom = (meeting?: TemporaryMeeting, tasks = [] as AppData['tasks']): MeetingDraft => meeting ? {
  ...
  taskDescription: meetingTaskDescription(meeting, tasks),
...
export default function TemporaryMeetingsPage({ data, visibleVessels, currentUser, commit }: Props) {
  ...
  const filtered = accessibleMeetings.filter(meeting => {
    ...
    if (typeFilter !== 'all') {
      if (scopeModeOf(meeting)==='all') return !q || `${meeting.subject} ${meeting.reason} ${meeting.resolution} ${meeting.taskDescription || ''} ${meeting.meetingDate} ${meetingScopeLabel(meeting)}`.toLowerCase().includes(q);
      if (!meetingVesselTypes(meeting).includes(typeFilter)) return false;
    }
    return !q || `${meeting.subject} ${meeting.reason} ${meeting.resolution} ${meeting.taskDescription || ''} ${meeting.meetingDate} ${meetingScopeLabel(meeting)}`.toLowerCase().includes(q);
  });
  ...
  const save = () => {
    ...
    const preserveExistingDescriptions = shouldPreserveMeetingTaskDescriptions(selected, data.tasks, savedDraft.taskDescription);
    commit(draftData => {
      ...
      const reconciliation = reconcileMeetingTasks({
        ...
        followUp: savedDraft.taskDescription,
  ...
  return <section className="temporary-meeting-page">
    ...
    {viewMode === 'register' ? <section className="panel meeting-register">
      <div className="panel-title"><div><h2>臨會/專題總清單</h2><p className="muted">共 {accessibleMeetings.length} 筆，目前篩選顯示 {filtered.length} 筆</p></div></div>
      ...
      <section className="meeting-column temporary-editor-column">
        ...
        <fieldset disabled={!editable} className={`column-scroll temporary-form ${!editable?'readonly-form':''}`} aria-readonly={!editable}>
          <div className="grid cols-3">
            ...
            <div className="field span-3"><label>待辦事項</label><textarea value={draft.taskDescription} onChange={event => setDraft({ ...draft, taskDescription: event.target.value })} placeholder="填寫後保存，會依實際船舶範圍建立或同步待辦" /></div>
          </div>
      ...
      <aside className="meeting-column temporary-summary-column">
        ...
        <div className="column-scroll">
          ...
          <div className="summary-card mint"><h3>關聯待辦事項</h3>{linkedTasks.length ? <div className="meeting-linked-tasks">{linkedTasks.map(task => <article key={task.id}><b>{vesselDisplayName(vesselById[task.vesselId])}</b><p>{task.description}</p><small>{task.isClosed ? '已結案' : task.status || '待執行'}｜期限 {task.expectedDate || '未設定'}</small></article>)}</div> : <p>{draft.taskDescription.trim() ? '保存後將依船舶範圍建立待辦。' : '尚未填寫待辦事項。'}</p>}</div>
          <div className="summary-card blue"><h3>待辦同步規則</h3><p>填寫待辦事項並保存後，每艘實際範圍船舶建立一筆待辦；再次保存會同步內容且不重複新增。</p></div>
        </div>
      </aside>
    </div>}
