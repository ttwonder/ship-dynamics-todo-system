...
type MeetingDraft = Pick<
  TemporaryMeeting,
  'subject' | 'meetingDate' | 'vessels' | 'reason' | 'departments' | 'participantUserIds' | 'responsibleUserIds' | 'resolution' | 'taskItems' | 'expectedDate' | 'priority'
> & {
...
export default function TemporaryMeetingsPage({ data, visibleVessels, currentUser, canExportReports, setData, commit }: Props) {
  ...
  const filtered = accessibleMeetings.filter(meeting => {
    ...
    return !q || `${meeting.subject} ${meeting.reason} ${meeting.resolution} ${[...meeting.participantUserIds, ...meeting.responsibleUserIds].map(id => users[id]?.name || '').join(' ')} ${meetingTaskItems(meeting, data.tasks).map(item => item.description).join(' ')} ${meeting.meetingDate} ${meetingScopeLabel(meeting)}`.toLowerCase().includes(q);
  });
  ...
  const save = () => {
    ...
    if (!draft.resolution.trim()) return alert('請填寫會議決議');
    ...
    if (!draft.participantUserIds.length) return alert('請至少選擇一位涉及人員');
    ...
    flushSync(()=>setData(prev=>{
      ...
      if(invalidParticipant){failure='涉及人員已停用或不存在，請重新選擇';return prev;}
    ...
    setDraft({...persistedDraft,taskItems:persistedDraft.taskItems.length?persistedDraft.taskItems:[{id:uid('meeting-task-item'),description:''}]});
  ...
  return <><section className="temporary-meeting-page meeting-screen">
    ...
    {viewMode === 'register' ? <section className="panel meeting-register">
      <div className="panel-title"><div><h2>臨會/專題總清單</h2><p className="muted">共 {accessibleMeetings.length} 筆，目前篩選顯示 {filtered.length} 筆{canExportReports ? `｜已選 ${selectedExportMeetings.length} 筆` : ''}</p></div>{canExportReports&&<div className="heading-actions no-print"><button className="btn small ghost" onClick={() => setMeetingExportSelection(Array.from(new Set([...meetingExportSelection, ...filtered.map(meeting => meeting.id)])))}>全選目前篩選</button><button className="btn small ghost" onClick={() => setMeetingExportSelection([])}>清空</button><button className="btn small primary" onClick={() => printMeetings('meetings')}>匯出所選會議 PDF</button><button className="btn small green" onClick={() => printMeetings('register')}>匯出總清單 PDF</button></div>}</div>
      ...
      <section className="meeting-column temporary-editor-column">
        ...
        <fieldset disabled={!editable} className={`column-scroll temporary-form ${!editable?'readonly-form':''}`} aria-readonly={!editable}>
          <div className="grid cols-3">
            ...
            <div className="field span-3"><label>決議／會議結論 <span className="required-mark">*</span></label><textarea required aria-required="true" value={draft.resolution} onChange={event => setDraft({ ...draft, resolution: event.target.value })} placeholder="記錄本次會議決議或結論" /></div>
          ...
          <div className="temporary-picker meeting-people-section">
            <MeetingPeoplePicker label="涉及人員" required users={meetingPeople} departments={data.settings.departments} selectedIds={draft.participantUserIds} onChange={participantUserIds => setDraft(previous => ({ ...previous, participantUserIds }))} />
      ...
      <aside className="meeting-column temporary-summary-column">
        ...
        <div className="column-scroll">
          ...
          <div className="summary-card blue"><h3>目前會議</h3><div className="summary-line"><span>狀態</span><b>{draft.status}</b></div><div className="summary-line"><span>範圍</span><b>{scopeModeLabel(draft.vesselScopeMode)}</b></div><div className="summary-line"><span>船舶</span><b>{resolvedVesselIds.length}</b></div><div className="summary-line"><span>部門</span><b>{draft.departments.length}</b></div><div className="summary-line"><span>涉及人員</span><b>{draft.participantUserIds.length}</b></div><div className="summary-line"><span>負責人</span><b>{draft.responsibleUserIds.length}</b></div><div className="summary-line"><span>關注</span><b>{draft.priority}</b></div></div>
  ...
  {printMode&&<section className="meeting-print print-only">
    {printMode==='meetings'&&selectedExportMeetings.map(meeting=>{const vesselIds=meetingVesselIds(meeting);const vesselNames=vesselIds.map(id=>vesselDisplayName(vesselById[id]));const items=meetingTaskItems(meeting,data.tasks);return <article className="meeting-print-page" key={meeting.id}><header><h1>臨會／專題會議報告</h1><p>匯出時間：{new Date().toLocaleString('zh-TW')}｜匯出人：{currentUser.name}</p></header><div className="meeting-print-meta"><div><small>會議主題</small><b>{meeting.subject||'-'}</b></div><div><small>狀態</small><b>{statusOf(meeting)}</b></div><div><small>召開日期</small><b>{meeting.meetingDate||'-'}</b></div><div><small>預計完成</small><b>{meeting.expectedDate||'-'}</b></div><div><small>關注程度</small><b>{meeting.priority}</b></div><div><small>會議範圍</small><b>{meetingScopeLabel(meeting)}</b></div></div><section><h2>涉會船舶</h2><p>{vesselNames.join('、')||'未指定'}</p></section><section><h2>涉及部門</h2><p>{meeting.departments.join('、')||'未指定'}</p></section><section><h2>涉及人員</h2><p>{peopleNames(meeting.participantUserIds)}</p></section><section><h2>負責人</h2><p>{peopleNames(meeting.responsibleUserIds)}</p></section><section><h2>召開緣由</h2><p>{meeting.reason||'未填寫'}</p></section><section><h2>決議／會議結論</h2><p>{meeting.resolution||'未填寫'}</p></section><section><h2>待辦事項</h2>{items.length?<ol>{items.map(item=><li key={item.id}>{item.description||'未填寫'}</li>)}</ol>:<p>尚無待辦事項</p>}</section></article>;})}
    {printMode==='register'&&<article className="meeting-print-register"><header><h1>臨會／專題總清單</h1><p>匯出時間：{new Date().toLocaleString('zh-TW')}｜匯出人：{currentUser.name}｜共 {accessibleMeetings.length} 筆</p></header><table><thead><tr><th>召開日期</th><th>狀態</th><th>主題</th><th>範圍</th><th>船舶</th><th>部門</th><th>涉及人員／負責人</th><th>待辦</th><th>期限</th></tr></thead><tbody>{accessibleMeetings.map(meeting=>{const vesselIds=meetingVesselIds(meeting);return <tr key={meeting.id}><td>{meeting.meetingDate||'-'}</td><td>{statusOf(meeting)}</td><td><b>{meeting.subject||'-'}</b><br/>{meeting.reason||'未填召開緣由'}</td><td>{meetingScopeLabel(meeting)}</td><td>{vesselIds.length} 艘<br/>{vesselIds.map(id=>vesselDisplayName(vesselById[id])).join('、')}</td><td>{meeting.departments.join('、')||'-'}</td><td>{peopleNames(meeting.participantUserIds)}<br/>負責：{peopleNames(meeting.responsibleUserIds)}</td><td>{meetingTaskCount(meeting.id)} 件</td><td>{meeting.expectedDate||'-'}</td></tr>;})}</tbody></table></article>}
  </section>}
  </>;
}
