...
export type ShipInternalControlPending = {
  version: 1; vesselId: string; actorKey: string; operationId: string; items: ShipInternalControlItem[];
};
...
export function readShipInternalControlDraft(storage: Pick<Storage, 'getItem'>, key: string): ShipInternalControlDraftRecord | null {
  try {
    ...
    if (record.pending !== undefined) {
      ...
      if (!object(pending) || pending.version !== 1 || pending.vesselId !== record.vessel.id
        ...
        || JSON.stringify(prepareShipInternalControlSubmission(record.draft as InternalControlBatchDraft, pending.actorKey, pending.operationId)) !== JSON.stringify(pending)) throw new Error();
    }
