...
type VesselDraft = Pick<Vessel, 'name' | 'shortName' | 'fullName' | 'shipType' | 'fleetCategory' | 'isActive' | 'assignedUserIds' | 'delegateManagers'> & VesselParticularDraft;
...
type Props = {
  ...
  onPrivateDraftChange?: ManagementDraftReporter;
  onSaveSupabaseConfig: (config:SupabaseConfig, reload?:{prepare:()=>boolean;committed:()=>void})=>Promise<boolean>;
};
...
const userDraft = (u?: UserAccount, department = '', vessels: Vessel[] = []): UserDraft => u ? {
  ...
  password: '',
  delegateVessels: vessels.filter(vessel => vessel.delegateManagers?.some(delegate => delegate.userId === u.id)).map(vessel => ({ vesselId: vessel.id, isActive: Boolean(vessel.delegateManagers.find(delegate => delegate.userId === u.id)?.isActive) })),
} : { department, name: '', username: '', role: 'operator', isActive: true, managedVesselIds: [], password: '', delegateVessels: [] };
const vesselDraft = (v?: Vessel): VesselDraft => v ? {
  name: v.name,
  shortName: v.shortName,
  fullName: v.fullName,
  shipType: v.shipType,
  ...vesselParticularDraft(v),
  fleetCategory: v.fleetCategory,
  isActive: v.isActive,
  assignedUserIds: [...(v.assignedUserIds || [])],
  delegateManagers: [...(v.delegateManagers || [])],
} : { name: '', shortName: '', fullName: '', shipType: '', fleetCategory: 'tanker fleet', isActive: true, assignedUserIds: [], delegateManagers: [], ...vesselParticularDraft() };
...
export default function ManagementView({ data, currentUser, commit, captureCommitContext, onPrivateDraftChange, onSaveSupabaseConfig }: Props) {
  const privateDrafts = useRef(new Map<object, 'persistent' | 'categories'>());
  const reportDraft = (scope: 'persistent' | 'categories'): ManagementDraftReporter => (token, dirty) => {
    if (dirty) { privateDrafts.current.set(token, scope); setSaveNotice(''); } else privateDrafts.current.delete(token);
    onPrivateDraftChange?.(token, dirty);
  };
  const allowDiscard = (dirty: boolean) => !dirty || confirmManagementDraftDiscard();
  ...
  const site = useManagementDraft(() => '', reportDraft('persistent'));
  const sitePassword=site.value, setSitePassword=site.edit;
  const connection = useManagementDraft<SupabaseConfig>(() => getSupabaseConfig() || emptyConfig, reportDraft('persistent'));
  const config = connection.value, setConfig = connection.edit;
  const saveConfig = async (value: SupabaseConfig) => {
    const draft = connection.capture(), current = captureContinuation();
    const peers = [person, ship, site], captures = peers.map(editor => editor.capture());
    const saved = await onSaveSupabaseConfig(value, {
      prepare: () => current() && draft.unchanged() && captures.every(peer => peer.unchanged()) && allowDiscard(peers.some(peer => peer.isDirty())),
      // Exact pre-write ownership was checked synchronously; config may now differ.
      committed: () => { draft.clean(); captures.forEach(peer => peer.clean()); },
    });
    if (saved && current() && draft.unchanged()) draft.clean();
    return saved;
  };
  ...
  const selectUser = (id: string) => {
    ...
    if (!creatingUser && id === selectedUserId) return true;
  ...
  const selectVessel = (id: string) => {
    ...
    if (!creatingVessel && id === selectedVesselId) return true;
    if (!allowDiscard(ship.isDirty())) return false;
  ...
  const startNewUser = () => {
    ...
    if (!allowDiscard(person.isDirty())) return;
  ...
  const startNewVessel = () => {
    ...
    if (!allowDiscard(ship.isDirty())) return;
  ...
  const savePerson = async () => {
    ...
    const saved=await commit(d => {
      ...
      d.vessels.forEach(v => {
        ...
        v.delegateManagers = (v.delegateManagers || []).filter(delegate => delegate.userId !== id);
  ...
  const clearPersonPassword = async () => {
    ...
    if(!saved||!continuation.unchanged())return;
    // Resetting the stored password did not submit the other private fields.
    person.refresh({ ...personDraft, password: '' });
  ...
  return <section className="management-view">
    ...
    <div className="management-shell">
      ...
      {section === 'directory' && <>
        ...
        <div className="management-detail">{selectedDirectory ? <DirectoryDetail item={selectedDirectory} data={data} onEdit={() => { if (selectedDirectory.kind === 'user') { if (selectUser(selectedDirectory.key.slice(5))) go('people'); } else { if (selectVessel(selectedDirectory.key.slice(7))) go('vessels'); } }}/>:<EmptyDetail text="沒有符合條件的資料"/>}</div>
      </>}
      ...
      {section === 'vessels' && <>
        ...
        <div className="management-detail"><VesselEditor draft={shipDraft} setDraft={next=>{setSaveNotice('');setShipDraft(next);}} creating={creatingVessel} referenceSuggested={!creatingVessel && vesselParticularDraft(data.vessels.find(v=>v.id===selectedVesselId)).particularsSuggested} users={activeUsers.filter(canManageVesselAssignments)} assignmentQuery={assignmentQuery} setAssignmentQuery={setAssignmentQuery} onSave={saveVessel} onDisable={disableVessel}/></div>
      </>}
      ...
      {section === 'categories' && canManageCategories && <>
        ...
        <div className="management-detail"><div className="management-category-stack"><TaskCategoryManager reportDraft={reportDraft('categories')} key="task" title="要事分類" subtitle="只套用於普通要事／早會來源待辦；歷史要事分類不會被改寫。" categories={data.settings.taskCategories} tasks={data.tasks.filter(task=>!isMeetingTaskSource(task))} onSave={async (categories,unchanged) => { const current=captureContinuation(); const saved=await commit(d => { d.settings.taskCategories = categories; d.settings.taskCategorySchemaVersion = 2; }, '更新要事分類', 'settings', 'task-categories', categories.join('、')); if(saved&&current()&&unchanged())setSaveNotice('要事分類已保存');return saved&&current(); }}/><TaskCategoryManager reportDraft={reportDraft('categories')} key="meeting" title="臨會/專題待辦分類" subtitle="只套用於臨會/專題來源待辦；不會混入普通要事分類。" categories={data.settings.meetingTaskCategories} tasks={data.tasks.filter(task=>isMeetingTaskSource(task))} onSave={async (categories,unchanged) => { const current=captureContinuation(); const saved=await commit(d => { d.settings.meetingTaskCategories = categories; d.settings.meetingTaskCategorySchemaVersion = 2; }, '更新臨會/專題待辦分類', 'settings', 'meeting-task-categories', categories.join('、')); if(saved&&current()&&unchanged())setSaveNotice('臨會/專題待辦分類已保存');return saved&&current(); }}/><EquipmentSubcategoryManager reportDraft={reportDraft('categories')} key="equipment" categories={data.settings.equipmentFailureSubcategories} usage={value=>data.internalControlCases.filter(item=>item.equipmentSubcategory===value).length} onSave={async(categories,unchanged)=>{const current=captureContinuation();const saved=await commit(d=>{d.settings.equipmentFailureSubcategories=categories;d.settings.equipmentFailureSubcategorySchemaVersion=1;},'更新內控設備故障細項','settings','internal-control-equipment-subcategories',categories.join('、'));if(saved&&current()&&unchanged())setSaveNotice('設備故障細項已保存');return saved&&current();}}/></div></div>
      </>}
...
export function PersonEditor({ draft, setDraft, creating, owner, manager, currentUser, selectedUserId, departments, vessels, assignmentQuery, setAssignmentQuery, onSave, onDisable, onClearPassword }: { draft:UserDraft; setDraft:React.Dispatch<React.SetStateAction<UserDraft>>; creating:boolean; owner:boolean; manager:boolean; currentUser:UserAccount; selectedUserId:string; departments:string[]; vessels:Vessel[]; assignmentQuery:string; setAssignmentQuery:(v:string)=>void; onSave:()=>void; onDisable:()=>void; onClearPassword:()=>void }) {
  ...
  const visibleDelegateVessels = vessels.filter(v => !draft.managedVesselIds.includes(v.id)).filter(v => !assignmentQuery.trim() || `${v.shortName} ${v.fullName} ${v.name}`.toLowerCase().includes(assignmentQuery.trim().toLowerCase()));
  const departmentChoices = Array.from(new Set([draft.department, ...departments])).filter(department => department && department !== '船舶帳戶');
  const passwordRequired = draft.role === 'owner' || draft.role === 'admin';
  const canClearPassword = !creating && manager && selectedUserId !== currentUser.id && draft.role !== 'owner' && draft.role !== 'admin';
  const selectedDelegateVesselNames = draft.delegateVessels.map(delegate => {
    const vessel = vessels.find(item => item.id === delegate.vesselId);
    return vessel ? `${vesselDisplayName(vessel)}（${delegate.isActive ? '激活' : '未激活'}）` : '';
  }).filter(Boolean) as string[];
  const toggle = (id:string) => setDraft(prev => {
    const managedVesselIds = prev.role === 'owner' ? [] : prev.role === 'vessel' ? (prev.managedVesselIds.includes(id) ? [] : [id]) : prev.managedVesselIds.includes(id) ? prev.managedVesselIds.filter(x => x !== id) : [...prev.managedVesselIds, id];
    return { ...prev, managedVesselIds, delegateVessels: prev.delegateVessels.filter(delegate => !managedVesselIds.includes(delegate.vesselId)) };
  });
  const togglePersonDelegateVessel = (id:string) => setDraft(prev => {
    const exists = prev.delegateVessels.some(delegate => delegate.vesselId === id);
    return { ...prev, delegateVessels: exists ? prev.delegateVessels.filter(delegate => delegate.vesselId !== id) : [...prev.delegateVessels, { vesselId: id, isActive: false }] };
  });
  const togglePersonDelegateVesselActive = (id:string) => setDraft(prev => ({ ...prev, delegateVessels: prev.delegateVessels.map(delegate => delegate.vesselId === id ? { ...delegate, isActive: !delegate.isActive } : delegate) }));
  const delegateVesselState = (id:string): UserDelegateVesselDraft | undefined => draft.delegateVessels.find(delegate => delegate.vesselId === id);
  const canDelegateVessels = manager && draft.role !== 'owner' && draft.role !== 'vessel';
  return <div className="management-editor"><EditorHeading title={creating ? '新增人員' : draft.name || '人員設定'} subtitle={creating ? '建立帳號、角色與經管船舶' : `${draft.department}｜${roleLabel(draft.role)}`} actions={<>{!creating && manager && selectedUserId !== currentUser.id && <button className="btn danger" onClick={onDisable}>停用</button>}<button className="btn primary" onClick={onSave}>{creating ? '建立人員' : '保存變更'}</button></>}/><div className="management-summary-grid"><Summary label="角色" value={roleLabel(draft.role)}/><Summary label="資料範圍" value={draft.role==='owner'?'全船隊':draft.role==='admin'?`全船可見｜經管 ${draft.managedVesselIds.length} 艘｜代管 ${draft.delegateVessels.length} 艘`:`經管 ${draft.managedVesselIds.length} 艘｜代管 ${draft.delegateVessels.length} 艘`}/><Summary label="帳號狀態" value={draft.isActive ? '啟用' : '停用'}/></div><EditorSection title="基本資料與角色"><div className="management-form"><label>姓名<input disabled={!manager && selectedUserId !== currentUser.id} value={draft.name} onChange={e => setDraft(prev => ({...prev,name:e.target.value}))}/></label><label>用戶名<input disabled={!manager && selectedUserId !== currentUser.id} value={draft.username} onChange={e => setDraft(prev => ({...prev,username:e.target.value}))}/></label><label>部門<select aria-label="人員部門" disabled={!manager || draft.role === 'vessel'} value={draft.role === 'vessel' ? '船舶帳戶' : draft.department} onChange={e => setDraft(prev => ({...prev,department:e.target.value}))}>{draft.role === 'vessel' ? <option value="船舶帳戶">船舶帳戶</option> : departmentChoices.map(department => <option key={department} value={department}>{department}</option>)}</select></label><label>角色<select disabled={!manager || selectedUserId === currentUser.id} value={draft.role} onChange={e => setDraft(prev => { const role = e.target.value as UserRole; return {...prev,role,department:departmentAfterRoleChange(prev.department,role,departments),managedVesselIds:role==='owner'?[]:role==='vessel'?prev.managedVesselIds.slice(0,1):prev.managedVesselIds,delegateVessels:role==='owner'||role==='vessel'?[]:prev.delegateVessels}; })}><option value="operator">操作員</option><option value="vessel">船舶帳戶</option>{manager&&<option value="admin">管理員</option>}{owner&&<option value="owner">Owner</option>}</select></label></div></EditorSection>{manager && draft.role !== 'owner' && <EditorSection title="經管船舶"><AssignmentPicker query={assignmentQuery} setQuery={setAssignmentQuery} count={draft.managedVesselIds.length} onAll={() => setDraft(prev => { const managedVesselIds = prev.role==='vessel'?vessels.slice(0,1).map(v=>v.id):vessels.map(v=>v.id); return {...prev,managedVesselIds,delegateVessels:prev.delegateVessels.filter(delegate=>!managedVesselIds.includes(delegate.vesselId))}; })} onClear={() => setDraft(prev => ({...prev,managedVesselIds:[]}))}>{visibleAssignments.map(v => <label key={v.id} className={draft.managedVesselIds.includes(v.id) ? 'selected' : ''}><input type="checkbox" checked={draft.managedVesselIds.includes(v.id)} onChange={() => toggle(v.id)}/><span>{vesselDisplayName(v)}</span><small>{v.shipType || '未填船型'}</small></label>)}</AssignmentPicker></EditorSection>}{canDelegateVessels && <EditorSection title="代管船舶"><AssignmentPicker query={assignmentQuery} setQuery={setAssignmentQuery} count={draft.delegateVessels.length} selectedNames={selectedDelegateVesselNames} onAll={() => setDraft(prev => ({...prev,delegateVessels:visibleDelegateVessels.map(v=>({vesselId:v.id,isActive:prev.delegateVessels.find(delegate=>delegate.vesselId===v.id)?.isActive ?? false}))}))} onClear={() => setDraft(prev => ({...prev,delegateVessels:[]}))}>{visibleDelegateVessels.map(v => { const delegate = delegateVesselState(v.id); return <div key={v.id} className={`delegate-manager-option ${delegate ? 'selected' : ''}`}><label><input type="checkbox" checked={Boolean(delegate)} onChange={() => togglePersonDelegateVessel(v.id)}/><span>{vesselDisplayName(v)}</span></label>{delegate && <button type="button" className={`delegate-manager-toggle ${delegate.isActive ? 'active' : 'inactive'}`} aria-label={`${vesselDisplayName(v)}｜${delegate.isActive ? '已激活；點擊切換為未激活' : '未激活；點擊切換為激活'}`} title={`${vesselDisplayName(v)}｜${delegate.isActive ? '已激活；點擊切換為未激活' : '未激活；點擊切換為激活'}`} onClick={() => togglePersonDelegateVesselActive(v.id)}><span className="delegate-state-dot" aria-hidden="true"/></button>}</div>; })}</AssignmentPicker></EditorSection>}<EditorSection title="密碼"><div className="management-password"><div><b>{creating ? passwordRequired ? '初始密碼（必填）' : '個人密碼（選填）' : owner&&selectedUserId!==currentUser.id ? 'Owner 重設密碼' : passwordRequired ? '重設密碼' : '個人密碼（選填）'}</b><small>{passwordRequired ? 'Owner／管理員登入必須使用密碼' : '個人密碼選填；未設定時可無密碼登入'}</small></div><input disabled={!creating&&selectedUserId!==currentUser.id&&!manager} type="password" value={draft.password} placeholder={creating ? passwordRequired ? '請設定 Owner／管理員密碼' : '可留空；留空則無密碼登入' : passwordRequired ? '留空表示不變更；不會顯示原密碼' : '留空表示不變更；設定後下次需輸入密碼'} onChange={e => setDraft(prev => ({...prev,password:e.target.value}))}/>{canClearPassword&&<button className="btn danger" onClick={onClearPassword}>清除密碼</button>}</div></EditorSection></div>;
}

function VesselEditor({ draft, setDraft, creating, referenceSuggested, users, assignmentQuery, setAssignmentQuery, onSave, onDisable }: { draft:VesselDraft; setDraft:React.Dispatch<React.SetStateAction<VesselDraft>>; creating:boolean; referenceSuggested:boolean; users:UserAccount[]; assignmentQuery:string; setAssignmentQuery:(v:string)=>void; onSave:()=>void; onDisable:()=>void }) {
  const [departmentFilter, setDepartmentFilter] = useState('all');
  ...
  const toggleDelegate = (id:string) => setDraft(prev => {
    ...
    return { ...prev, delegateManagers: exists ? prev.delegateManagers.filter(delegate => delegate.userId !== id) : [...prev.delegateManagers, { userId: id, isActive: false }] };
  });
...
function TaskCategoryManager({ title, subtitle, categories, tasks, onSave, reportDraft }: { reportDraft?:ManagementDraftReporter; title:string; subtitle:string; categories:string[]; tasks:AppData['tasks']; onSave:(categories:string[], unchanged:()=>boolean)=>Promise<boolean> }) {
  const editor=useManagementDraft(()=>[...categories],reportDraft), entry=useManagementDraft(()=>'',reportDraft);
  ...
  const save = () => {
    ...
    void onSave(clean,()=>submitted.unchanged()&&pendingEntry.unchanged()).then(saved=>{if(saved&&submitted.unchanged()&&pendingEntry.unchanged()){submitted.clean();if(!entry.value)pendingEntry.clean();}});
  };
...
function EquipmentSubcategoryManager({ categories, usage, onSave, reportDraft }: { reportDraft?:ManagementDraftReporter; categories:string[]; usage:(value:string)=>number; onSave:(categories:string[], unchanged:()=>boolean)=>Promise<boolean> }) {
  const editor=useManagementDraft(()=>[...categories],reportDraft),entry=useManagementDraft(()=>'',reportDraft);
  const draft=editor.value,setDraft=editor.edit,newValue=entry.value,setNewValue=entry.edit;
  useEffect(()=>editor.refresh([...categories]),[categories]);
  const add=()=>{const value=newValue.trim();if(!value)return alert('請輸入設備故障細項');if(draft.some(item=>item.toLocaleLowerCase()===value.toLocaleLowerCase()))return alert('細項名稱不可重複');setDraft(prev=>[...prev,value]);entry.initialize('');};
...
function OwnerSettings({ panel, sitePassword, setSitePassword, captureDraft, resetSitePassword, captureContinuation, captureCommitContext, config, setConfig, data, commit, onSaveSupabaseConfig }: { panel:'gate'|'supabase'|'cloud'; sitePassword:string; setSitePassword:(v:string)=>void; captureDraft:()=>{unchanged:()=>boolean;sameEditor:()=>boolean}; resetSitePassword:(v:string)=>void; captureContinuation:()=>()=>boolean; captureCommitContext:Props['captureCommitContext']; config:SupabaseConfig; setConfig:React.Dispatch<React.SetStateAction<SupabaseConfig>>; data:AppData; commit:Props['commit']; onSaveSupabaseConfig:Props['onSaveSupabaseConfig'] }) {
  const saveGate = async () => { if (!sitePassword) return alert('請輸入新進站密碼'); const draft=captureDraft(),current=captureContinuation(),transaction=captureCommitContext(); const hash = await sha256(sitePassword); if(!transaction()||!current()||!draft.sameEditor())return; const saved=await commit(d => { d.settings.sitePasswordHash = hash; }, '修改進站密碼', 'settings', 'site-password', 'Owner 更新進站密碼',transaction); if(!saved||!current()||!draft.unchanged())return; resetSitePassword(''); alert('進站密碼已更新'); };
