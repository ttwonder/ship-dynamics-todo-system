...
export default function MorningWorkspaceView({ data, user, visibleVessels, selected, setSelected, onEditTask, onAddTask, onOpenVessel, onOpenTemporaryMeeting, onOpenReport, commit }:Props) {
  ...
  const discussionTasks = data.tasks.filter(t => taskVesselIds(t).some(id => scopeSet.has(id)) && !t.isClosed).sort((a,b) => {
  ...
  const allScopeTasks = data.tasks.filter(t => taskVesselIds(t).some(id => scopeSet.has(id)));
