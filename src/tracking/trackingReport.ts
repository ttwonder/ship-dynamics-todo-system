...
export const compactTrackingColumns=(kind:TrackingKind)=>trackingColumnsFor(kind).filter(c=>['referenceNo','applicationDate','description','expectedDate','progress','supplementalNotes',kind==='supply'?'deliveryStatus':'originalRemarks'].includes(c.key));
