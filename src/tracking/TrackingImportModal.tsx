 ...
 return <div className="modal-backdrop"><div className="modal tracking-modal tracking-import" role="dialog" aria-modal="true" aria-label="Excel 導入預覽">
  ...
  {state.parsed&&<><label>來源工作表<select aria-label="匯入工作表" disabled={busy||Boolean(state.pending)} value={state.sheet} onChange={e=>write({...state,sheet:Number(e.target.value),shipConfirmed:false})}>{state.parsed.sheets.map((s,i)=><option key={s.name} value={i}>{s.name}｜{s.format}</option>)}</select></label>
   ...
   <div className="tracking-import-rows">{rows.map(row=>{const duplicate=importDuplicate(row,rows,data.trackingItems||[]),problems=errors(row),prefix=`匯入第 ${row.sourceRow} 列 `;const update=(patch:Partial<TrackingItem>)=>changeRow(row.key,{item:{...row.item,...patch}});return <fieldset key={row.key} disabled={busy||Boolean(state.pending)||row.saved} className="tracking-form-row"><legend><label><input aria-label={`選取來源第${row.sourceRow}列`} type="checkbox" checked={row.selected} disabled={duplicate.blocked||row.saved} onChange={e=>changeRow(row.key,{selected:e.target.checked})}/>來源第 {row.sourceRow} 列｜{row.item.referenceNo} {row.item.subitemNo&&`分項 ${row.item.subitemNo}`}｜{row.saved?'已確認保存':problems.length?'需處理':'可匯入'}</label></legend>
    ...
    <details open={row.selected&&Boolean(problems.length)}><summary>核對／編輯完整欄位及原值</summary><div className="tracking-form-grid">{trackingColumnsFor(row.item.kind).filter(c=>c.editable||['progress','actualDeliveryDate'].includes(c.key)).map(col=><label key={col.key}>{col.label}{col.required?' *':''}{col.type==='date'?<input type="date" aria-label={prefix+col.label} value={col.value(row.item)} onChange={e=>update({[col.key]:e.target.value})}/>:<textarea rows={2} aria-label={prefix+col.label} value={col.value(row.item)} onChange={e=>update({[col.key]:col.key==='urgentSubtypes'?e.target.value.split('、'):e.target.value})}/>}</label>)}
     <label>緊急度<select aria-label={prefix+'緊急度'} value={row.item.urgency} onChange={e=>update({urgency:e.target.value as TrackingItem['urgency']})}><option value="normal">普通</option><option value="urgent">緊急</option></select></label>
     ...
     {row.item.isClosed&&<><label>結案日期<input type="date" aria-label={prefix+'結案日期'} value={row.item.closedDate||''} onChange={e=>update({closedDate:e.target.value})}/></label>{row.item.kind==='engineering'&&<><label>結案結果<select aria-label={prefix+'結案結果'} value={row.item.closureOutcome||'completed'} onChange={e=>update({closureOutcome:e.target.value as TrackingItem['closureOutcome']})}><option value="completed">完工</option><option value="cancelled">取消（非完工）</option></select></label><button type="button" className="btn small" disabled={!row.item.completionDate} onClick={()=>update({closedDate:row.item.completionDate})}>明確採用已核對完工日期作結案日期</button></>}</>}
