...
export default function InternalControlPage({ editorOnly=false, loadCase, data, user, vessels, canCreate, canEdit, canClose, canDelete, canExport, authorizationEpoch, requestedCaseId, onRequestedCaseHandled, onCreate, onUpdate, onWithdrawTaskSync, onDelete, onBatchClose, onBatchDelete, onOpenTask, claimItemLease, requireItemLease, releaseItemLease, activeItemLeaseKey }: Props) {
  ...
  const stats = buildInternalControlStats(filtered, vessels, vesselSelectionDisplayName);
  ...
  const selectedVesselNames = filters.vesselIds.map(id => vessels.find(vessel => vessel.id === id)).filter((vessel): vessel is Vessel => Boolean(vessel)).map(vesselSelectionDisplayName);
  ...
  return <section className="internal-control-page">
    ...
    {subpage !== 'stats' ? <section className="panel ic-list-panel">
      ...
      <div className="table-wrap"><table className="compact ic-table"><thead><tr>
        ...
        return <tr key={item.id} className={selectedSet.has(item.id)?'batch-selected-row':''}>
          ...
          <td><b>{vessel ? vesselSelectionDisplayName(vessel) : item.vesselId}</b><small>{vessel?.shipType || '未填船型'}｜{item.reportDate}</small></td>
    ...
    </section> : <InternalControlStatsView stats={stats} cases={filtered} vessels={vessels} fromDate={filters.fromDate} toDate={filters.toDate} filterSummary={analyticsFilterSummary} selection={analysisSelection} onSelectionChange={setAnalysisSelection} formatVesselName={vesselSelectionDisplayName}/>}
