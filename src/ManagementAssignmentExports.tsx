...
export function ManagementAssignmentPaper({ report }: { report: ManagementAssignmentReport }) {
  ...
  return <article ref={paper} className="management-assignment-paper">
    ...
    <table><colgroup>{widths.map((width, index) => <col key={index} style={{ width: `${width / total * 100}%` }}/>)}</colgroup>
      <thead><tr><th rowSpan={2}>{text('船隊')}</th><th rowSpan={2}>{text('船型')}</th><th colSpan={2}>{text('船名')}</th><th colSpan={report.departments.length}>{text('分管部門／人員')}</th><th rowSpan={2}>{text('年分')}</th><th rowSpan={2}>{text('噸數')}</th></tr><tr><th>{text('中文')}</th><th>{text('英文')}</th>{report.departments.map(department => <th key={department}>{text(department)}</th>)}</tr></thead>
