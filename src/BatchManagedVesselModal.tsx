...
type Props = {
  ...
  editableVesselId: string;
  onBeginEdit: (vessel: Vessel) => void;
  commit: Commit;
  close: () => void;
...
export default function BatchManagedVesselModal({ vessels, currentUser, editableVesselId, onBeginEdit, commit, close, onAddTask }: Props) {
  ...
  const updateVessel = (vesselId: string, detail: string, mutate: (vessel: Vessel) => void) => {
    if(editableVesselId!==vesselId)return;
    commit(draft => {
