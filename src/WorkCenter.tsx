...
const workTaskLabel=(task:TaskItem)=>task.internalControlCaseId?'內控(已同步要事)':task.sourceMeetingId||task.sourceType==='temporary'?'會議待辦':'普通要事';
...
export default function WorkCenter({batchContext:providedBatchContext,data,user,vessels,onOpenTask,onOpenInternalControl,onOpenVessel,markAllRead,canComplete,canDelete,canPrint,onPrint,onBatchComplete,onDismiss,onBatchDelete}:Props){
  ...
  const [taskSource,setTaskSource]=useState<'all'|'morning'|'temporary'|'internal'|'internal-synced'>('all');
  ...
  const filteredTasks=allTasks.filter(task=>{
    if(taskSource==='internal'||(taskSource==='internal-synced'&&!task.internalControlCaseId))return false;
    ...
    if(taskSource!=='all'&&taskSource!=='internal-synced'&&source!==taskSource)return false;
  ...
  return <section className="work-center">
    ...
    <section className="panel work-task-panel"><div className="panel-title"><h2>我的待辦清單 <span className="muted">({filteredTasks.length+filteredInternalCases.length}/{allTasks.length+allInternalCases.length})</span></h2><div className="heading-actions no-print">{hasTaskFilters&&<button className="btn small ghost" onClick={resetTaskFilters}>清除篩選</button>}<button className="btn small ghost" onClick={toggleAll} disabled={!selectableCount}>{allSelected?'取消全選':'全選目前結果'}</button><span className="batch-selection-count">已選 {selectionCount}</span><button className="btn small ghost" onClick={dismissSelected} disabled={!selectionCount}>從我的待辦移除（{selectionCount}）</button><button className="btn small green" onClick={completeSelected} disabled={!canComplete||!completeSelectionCount} title={!canComplete?'目前角色未獲授權批量完成':!completeSelectionCount&&selectionCount?'分派到多船的會議待辦請逐船完成':''}>批量完成（{completeSelectionCount}）</button>{canDelete&&<button className="btn small red" onClick={deleteSelected} disabled={!selectionCount} title="永久刪除共用待辦及其關聯資料，執行前會再次確認影響範圍">永久刪除共用待辦（{selectionCount}）</button>}{canPrint&&<button className="btn small primary" onClick={onPrint} disabled={!selectionCount} title={!selectionCount?'請先勾選要輸出的項目':''}>導出 PDF（{selectionCount}）</button>}</div></div>
      ...
      <section className="work-print-list print-only"><h1>我的待辦清單（所選項目）</h1><p>匯出人：{user.name}｜匯出時間：{formatTaipeiDateTime(new Date())}｜所選 {printTasks.length+printInternalCases.length} 件</p><table><colgroup><col className="print-col-source"/><col className="print-col-vessel"/><col className="print-col-attention"/><col className="print-col-item"/><col className="print-col-department"/><col className="print-col-deadline"/><col className="print-col-status"/></colgroup><thead><tr><th>來源</th><th>船舶</th><th>關注</th><th>事項</th><th>部門</th><th>期限</th><th>狀態</th></tr></thead><tbody>{printInternalCases.map(item=>{const vessel=vessels.find(entry=>entry.id===item.vesselId);return <tr key={`internal-${item.id}`}><td>內控(未同步要事)</td><td>{vessel?pdfVesselDisplayName(vessel):item.vesselId}</td><td>{item.priority}</td><td>{richTextToPlainText(item.description)||'未命名內控事項'}</td><td>{item.departments.join('、')||'未指定'}</td><td>—</td><td>{richTextToPlainText(item.status)||'尚未更新'}</td></tr>;})}{printTasks.map(task=>{const scopedIds=taskVesselIds(task).filter(id=>visibleVesselIds.has(id));const summary=taskVesselProgressSummary(task,scopedIds);const projected=taskProjectedProgressForScope(task,scopedIds);return <tr key={task.id}><td>{workTaskLabel(task)}{usesPerVesselProgress(task)?`｜單船 ${summary.completed}/${summary.total}`:''}</td><td>{taskVesselLabel(task,vessels,pdfVesselDisplayName)}</td><td>{task.priority}</td><td>{richTextToPlainText(task.description)||'未命名事項'}</td><td>{task.departments.join('、')||'未指定'}</td><td>{task.expectedDate||'未設定'}</td><td>{richTextToPlainText(projected.status)||'尚未更新'}</td></tr>;})}</tbody></table></section>
    </section>
  </section>;
}
