...
type TaskSort='created-desc'|'priority'|'due-asc'|'due-desc'|'updated-desc'|'vessel';
...
type Props={data:AppData;user:UserAccount;vessels:Vessel[];onOpenTask:(task:TaskItem)=>void;onOpenInternalControl:()=>void;onOpenVessel:(vesselId:string)=>void;markAllRead:()=>void|Promise<void>;canComplete:boolean;canDelete:boolean;canPrint:boolean;onPrint:()=>void;onBatchComplete:(ids:string[])=>boolean|Promise<boolean>;onBatchDelete:(taskIds:string[],internalControlCaseIds?:string[])=>boolean|Promise<boolean>};
...
export default function WorkCenter({data,user,vessels,onOpenTask,onOpenInternalControl,onOpenVessel,markAllRead,canComplete,canDelete,canPrint,onPrint,onBatchComplete,onBatchDelete}:Props){
  ...
  const workRows=[
    ...
    if(taskSort==='due-asc'||taskSort==='due-desc'){
      ...
      return compareOptionalDate(leftDate,rightDate,taskSort==='due-asc'?1:-1)||priorityRank[leftItem.priority]-priorityRank[rightItem.priority]||compareCreatedNewestFirst(leftItem,rightItem);
    }
    if(taskSort==='vessel'){
      ...
      return leftLabel.localeCompare(rightLabel,'zh-TW')||compareCreatedNewestFirst(leftItem,rightItem);
    }
  ...
  const selectableInternalCases=canDelete?filteredInternalCases:[];
  useEffect(()=>{
    setSelectedIds(previous=>{const next=sanitizeTaskSelection(previous,actionableTasks);return next.length===previous.length&&next.every((id,index)=>id===previous[index])?previous:next;});
    ...
    setPage(1);
  },[data.tasks,data.internalControlCases,taskQuery,taskVessel,taskPriority,taskSource,taskSort,user.id,vessels,canDelete]);
  ...
  const selectableCount=actionableTasks.length+selectableInternalCases.length;
  const allSelected=selectableCount>0&&actionableTasks.every(task=>selectedSet.has(task.id))&&selectableInternalCases.every(item=>selectedInternalSet.has(item.id));
  const toggleAll=()=>{setSelectedIds(allSelected?[]:actionableTasks.map(task=>task.id));setSelectedInternalCaseIds(allSelected?[]:selectableInternalCases.map(item=>item.id));};
  ...
  const hasTaskFilters=Boolean(taskQuery.trim()||taskVessel!=='all'||taskPriority!=='all'||taskSource!=='all'||taskSort!=='created-desc');
  const resetTaskFilters=()=>{setTaskQuery('');setTaskVessel('all');setTaskPriority('all');setTaskSource('all');setTaskSort('created-desc');};
