import { rebindReopenedVesselResponsibilities, changedVesselTeams, handoverReadTargets, applyVesselManagerHandover, canRetainVesselCommonContact, primaryVesselTeam } from './vesselManagerHandover';
...
import { pdfVesselDisplayName, vesselDisplayName, vesselSelectionDisplayName } from './vesselDisplay';
...
import { acquireEditLockBundle, type OwnedEditLockBundleLease } from './editLockBundle';
...
type Tab = 'tracking' | 'dashboard' | 'morning' | 'total' | 'reports' | 'stats' | 'management' | 'meeting' | 'closed' | 'internalControl' | 'work';
type ActiveEditLock = { sectionKey: string; label: string; status: 'owned' | 'blocked' | 'error'; ownerUserId: string; ownerUserName: string; leaseOwnerId: string; generation: number; authorizationEpoch: string; validatedUntilMs: number; lockedByName?: string; bundle?: OwnedEditLockBundleLease[]; bundleSeeds?: string[] };
...
export function taskMatchesFilters(t: TaskItem, filters: FilterState, vesselMap: Record<string,Vessel>, currentUser: UserAccount | null, applyClosedMode: boolean, canViewAll = false, taskOwnerAccess = false) {
...
export default function App() {
  ...
  const trackingNavigationGuard=useRef<null|(()=>Promise<boolean>)>(null);
  const trackingCreationAdmissions=useRef(new Map<string,{vesselId:string;actorId:string;identity:string}>());
  const trackingAttempt=useRef<{submission:TrackingSubmission;sectionKey:string;applied:boolean;confirmed:boolean}|null>(null);
  const trackingIdentity=()=>JSON.stringify([liveCurrentUserId.current,identitySessionGeneration.current,liveAuthorizationEpoch.current,cloudConfigIdentity(getSupabaseConfig())]);
  ...
  const liveListBatchView=useRef(tab);
  const listBatchViewGeneration=useRef(0);
  if(liveListBatchView.current!==tab)listBatchViewGeneration.current+=1;
  liveListBatchView.current=tab;
  ...
  const realtimeWakeupState=(incomingRevision:number)=>({
    ...
    hasActiveItemLease:Boolean(activeEditLockRef.current||actionScopeReadInFlight.current||pendingClaimConfig.current),
  ...
  const captureCloudBlockLockGuards=(config:ResolvedSupabaseConfig):CloudBlockLockGuard[]=>{
    ...
    for(const lease of active?.bundle||[])add({...active!,...lease},leaseCloudConfigs.current.get(lease.leaseOwnerId));
  ...
  const enqueueCloudSave = (snapshot: AppData,isCurrent:()=>boolean=()=>true,renderRebase=true,canSubmit:()=>boolean=isCurrent,handoverReadRevision?:number,authoritySave?:{binding:BrowserAuthority;base:AppData},onRecordPrepared?:(attempt:TrackingRecordAttempt)=>void): Promise<void> => {
          ...
          try{
            ...
            while (pendingEntry) {
              ...
              while(!persisted){
                ...
                const strictAuthorizationGuard=pending.handoverReadRevision!==undefined
                  ? {...authorizationDomainGuard(storageRemote),handoverReadRevision:pending.handoverReadRevision}
                  : appDataAuthorizationDomainChanged(remote,candidate)?authorizationDomainGuard(storageRemote):null;
                const runWithCurrentRecoveryLocks=(run:(guards:CloudBlockLockGuard[])=>Promise<AppData>)=>runWithCloudSaveRecoveryLocks({
                  ...
                  stillCurrent:()=>canSubmit()&&isCurrent()&&pendingActorIsCurrent()&&configIoCoordinator.current.isCurrent(token,getSupabaseConfig())&&hasCurrentCloudIdentity()&&(!legacyWholeStateFallback||(saveTurnOwned&&saveTurnHeartbeat.isActive())),
                ...
                if(!legacyWholeStateFallback){
                  try{
                    ...
                    const recovery=await runWithCurrentRecoveryLocks(async guards=>{
                      ...
                      try{
                        pending.onRecordPrepared?.({operationId,operations:clone(operations),savedBy,actorId:actorUserId,actorGuard:clone(actorGuard),authorizationGuard:strictAuthorizationGuard?clone(strictAuthorizationGuard):null,guards:clone(guards),scope:commandReadScope,authority:pending.authority,visibleSnapshot:clone(liveData.current)});
  ...
  // Continuity keeps the exact handoff identity; error/TTL revokes writes, not the draft.
  const freezeRelatedMutationDraft=(lock:ActiveEditLock|null)=>{
    if(!lock||(!lock.bundle&&!relatedMutationHandoffMatchesCurrent(lock))||(!lock.sectionKey.startsWith('task:')&&!lock.sectionKey.startsWith('internal-control:')&&!lock.sectionKey.startsWith('tracking:')))return false;
    const frozen={...lock,status:'error' as const,validatedUntilMs:0};
    activeEditLockRef.current=frozen;
    setActiveEditLock(previous=>relatedMutationLeaseMatches(lock,previous)?frozen:previous);
    return true;
  };
  const releaseCurrentEditLock=async () => {
    ...
    const leaseRecord=leaseCloudConfigs.current.get(lock.leaseOwnerId)||lock.bundle?.map(lease=>leaseCloudConfigs.current.get(lease.leaseOwnerId)).find(Boolean);
    if(!leaseRecord){
      activeEditLockRef.current=null;
      setActiveEditLock(previous=>previous?.leaseOwnerId===lock.leaseOwnerId?null:previous);
      return true;
    ...
    try{
      await lockCoordinator.current.run(async()=>{
        for(const lease of [...(lock.bundle||[{sectionKey:lock.sectionKey,leaseOwnerId:lock.leaseOwnerId}])].reverse()){
          const record=leaseCloudConfigs.current.get(lease.leaseOwnerId);
          if(!record)continue;
          await runCloudSaveQueueRpc('釋放多人協作鎖',signal=>releaseEditLock(lease.sectionKey,lease.leaseOwnerId,record.config,signal),8_000);
          leaseCloudConfigs.current.delete(lease.leaseOwnerId);
        }
      });
      leaseCloudConfigs.current.delete(lock.leaseOwnerId);
  ...
  const resolveEditLockNotice=async()=>{
    const lock=activeEditLock;
    if(!lock)return;
    if(relatedMutationHandoffMatchesCurrent(lock)){
      if(await releaseExclusiveItemLease(lock.sectionKey))closeEditorForLock(lock);
      return;
    }
  ...
  useEffect(()=>{
    const lock=activeEditLock;
    const checkConfig=()=>{
      memberEditor.current?.checkCurrent();
    ...
    const renewSingleItemLease=()=>{
      ...
      void lockCoordinator.current.run(async()=>{
        ...
        try{
          ...
          const renewals=await Promise.all((lock.bundle||[{sectionKey:lock.sectionKey,leaseOwnerId:lock.leaseOwnerId}]).map(lease=>runCloudSaveQueueRpc('單項及關聯協作鎖續期',signal=>renewEditLock(lease.sectionKey,lease.leaseOwnerId,75,leaseRecord.config,signal),8_000)));
          const renewed=renewals.find(value=>!value.ok)||{ok:true,lockedByName:undefined,expiresAt:new Date(Math.min(...renewals.map(value=>Date.parse(value.expiresAt||'')))).toISOString()};
          ...
          if(!renewed.ok){
            if(freezeRelatedMutationDraft(lock))return;
          ...
          if(!freezeRelatedMutationDraft(lock))setActiveEditLock({...lock,status:'error'});
  ...
  const claimEditingLock=async(sectionKey:string,label:string,stillWanted?:()=>boolean,announceBlocked=true,liveAuthorized?:()=>boolean,selectedKeys?:readonly string[]):Promise<EditLockClaimResult>=>{
    ...
    try{return await lockCoordinator.current.run(async()=>{
      ...
      try{
        const grouped=/^(task:|internal-control:|tracking:|meeting:)/.test(sectionKey);
        let bundle:OwnedEditLockBundleLease[]|undefined;
        const seeds=[...new Set(selectedKeys||[sectionKey])].sort();
        const keysFor=(snapshot:AppData)=>[...new Set(seeds.flatMap(key=>relatedEntityLockKeysForSection(snapshot,key)))].sort();
        const current=()=>sameCloudConfig(getSupabaseConfig(),leaseConfig)&&(!stillWanted||stillWanted())&&lockCoordinator.current.isCurrent(generation)&&liveAuthorizationEpoch.current===claimAuthorizationEpoch&&authorizedNow();
        const releaseBundleLease=async(lease:{sectionKey:string;leaseOwnerId:string})=>{
          await runCloudSaveQueueRpc('回滾編輯進場鎖',signal=>releaseEditLock(lease.sectionKey,lease.leaseOwnerId,leaseConfig,signal),8_000);
          leaseCloudConfigs.current.delete(lease.leaseOwnerId);
        };
        const lock=await (async()=>{
          if(!grouped)return runCloudSaveQueueRpc('取得多人協作鎖',signal=>claimEditLock(sectionKey,leaseOwnerId,ownerUserName,75,leaseConfig,signal),8_000);
          const targets=seeds.flatMap(key=>{const at=key.indexOf(':'),prefix=key.slice(0,at),id=key.slice(at+1);return [{collection:({'task':'tasks','internal-control':'internalControlCases','tracking':'trackingItems','meeting':'meetings'} as const)[prefix as 'task'] ,id}];});
          const scope=unionRecordScopes(recordReadScope.current,{targets});
          const planning=await fetchCloudData(leaseConfig,undefined,undefined,scope);
          if(!planning||!current()||seeds.some(key=>!itemLeaseExistsInSnapshot(key,planning)||!itemLeaseIsAuthorizedInSnapshot(key,planning)))throw new Error('所選項目或編輯權限已變更');
          const planned=keysFor(planning);
          const requests=planned.map(key=>({sectionKey:key,label:`${label}｜${key}`,leaseOwnerId:key===sectionKey?leaseOwnerId:`${ownerUserId}:${crypto.randomUUID()}`}));
          const result=await acquireEditLockBundle(requests,request=>{
            leaseCloudConfigs.current.set(request.leaseOwnerId,{sectionKey:request.sectionKey,config:leaseConfig});
            return runCloudSaveQueueRpc('取得編輯進場整組鎖',signal=>claimEditLock(request.sectionKey,request.leaseOwnerId,ownerUserName,75,leaseConfig,signal),8_000);
          },releaseBundleLease,current);
          if(result.status==='blocked')return {ok:false,lockedByName:result.lockedByName};
          if(result.status!=='owned')throw new Error('未能完整取得編輯鎖；本次不開放輸入');
          bundle=result.leases;
          const fresh=await fetchCloudData(leaseConfig,undefined,undefined,scope);
        ...
        if(!configStillCurrent||(stillWanted&&!stillWanted())||!lockCoordinator.current.isCurrent(generation)||liveAuthorizationEpoch.current!==claimAuthorizationEpoch||!authorizedNow()){
          if(lock.ok){for(const lease of bundle||[{sectionKey,leaseOwnerId}])await releaseBundleLease(lease);}
        ...
        if(!lock.ok){
          ...
          activeEditLockRef.current={...lockState,status:'blocked',lockedByName};
          setActiveEditLock(activeEditLockRef.current);
  ...
  const commitManagement=(updater:(draft:AppData)=>void,action:string,entityType:string,entityId:string,detail:string,isCurrent=captureManagementContext()):Promise<boolean>=>{
    ...
    const operation=managementSaveTail.current.then(async()=>{
      ...
      if(changedTeams.length&&recordHandover){
        ...
        const targets=handoverReadTargets(discovered,changedVesselTeams(discovered,candidate));
        if(!await loadRecordActionScope({targets},isCurrent,true))return false;
        if(liveData.current.revision!==discovered.revision)throw new Error('交接讀取期間資料已變更，請重新保存');
        baseline=liveData.current;
      }
      const actor=baseline.users.find(user=>user.id===actorId&&user.isActive);
      if(!actor)throw new Error('目前身份已失效，請重新登入');
      const draft=clone(baseline);updater(draft);
      const handedOver=applyVesselManagerHandover(baseline,draft);
      // Legacy snapshot/local saves retain their existing atomic save route.
      const handoverDetail=handedOver.length?JSON.stringify(handedOver.map(vesselId=>({vesselId,before:primaryVesselTeam(baseline,vesselId),after:primaryVesselTeam(draft,vesselId),events:handoverReadTargets(draft,[vesselId]).filter(target=>{const previous=baseline[target.collection].find(row=>row.id===target.id),next=draft[target.collection].find(row=>row.id===target.id);return JSON.stringify((previous as {vesselResponsibilities?:unknown})?.vesselResponsibilities)!==JSON.stringify((next as {vesselResponsibilities?:unknown})?.vesselResponsibilities);} )} ))):'';
      const snapshot=withAudit(draft,actor,action,entityType,entityId,handoverDetail?`${detail}\n逐船責任交接：${handoverDetail}`:detail);
      if(!config){
  ...
  const requireMutationLease=(sectionKey:string)=>{
    ...
    const lock=activeEditLockRef.current;
    if(lock?.sectionKey===sectionKey&&freezeRelatedMutationDraft(lock))return false;
  ...
  const roleVisibleInternalControlCases=useMemo(()=>{
    if(!currentUser)return [];
    // Keep the original Page/Modal instance after an optimistic deletion. This is a
    // read projection only; activeItemLeaseKey still comes from actual write authority.
  ...
  const authorizedEditLockKeys=useMemo(()=>new Set<string>([
    ...
    ...(currentUser?.role!=='vessel'&&(canCreateTasks||canEditBusinessContent||canCloseTasks)?(data.trackingItems||[]).filter(item=>activeVessels.some(v=>v.id===item.vesselId)).map(item=>`tracking:${item.id}`):[]),
    ...(activeEditLock&&trackingCreationAdmissions.current.has(activeEditLock.sectionKey)&&canCreateTasks&&trackingCreationAdmissions.current.get(activeEditLock.sectionKey)?.actorId===currentUser?.id?[activeEditLock.sectionKey]:[]),
  ]),[canEditBusinessContent,canCreateTasks,canEditMeetings,canMutateInternalControl,currentUser,activeVessels,roleVisibleTasks,roleVisibleMeetings,roleVisibleInternalControlCases,data.trackingItems,data.settings.rolePermissions,activeEditLock?.sectionKey]);
  ...
  const itemLeaseIsAuthorizedInSnapshot=(sectionKey:string,snapshot:AppData)=>{
    ...
    const trackingAdmission=trackingCreationAdmissions.current.get(sectionKey);
    if(trackingAdmission&&!snapshot.trackingItems?.some(item=>`tracking:${item.id}`===sectionKey))return trackingAdmission.actorId===actor.id&&trackingAdmission.identity===trackingIdentity()&&actor.role!=='vessel'&&hasPermission(snapshot.settings.rolePermissions,actor,'createTasks')&&actorVessels.some(v=>v.id===trackingAdmission.vesselId);
    if(sectionKey.startsWith('tracking:'))return actor.role!=='vessel'&&(['createTasks','editBusinessContent','closeTasks','deleteTasks'] as const).some(p=>hasPermission(snapshot.settings.rolePermissions,actor,p))&&Boolean(snapshot.trackingItems?.some(item=>`tracking:${item.id}`===sectionKey&&actorVessels.some(v=>v.id===item.vesselId)));
    return false;
  };
  const itemLeaseExistsInSnapshot=(sectionKey:string,snapshot:AppData)=>{
    ...
    if(sectionKey.startsWith('tracking:'))return Boolean(snapshot.trackingItems?.some(item=>`tracking:${item.id}`===sectionKey)||trackingCreationAdmissions.current.has(sectionKey));
    return false;
  };
  const refreshAfterItemLease=async(sectionKey:string,vesselFreshness=false,ownerIsCurrent:()=>boolean=()=>true):Promise<AppData|null>=>{
    ...
    const claimStillCurrent=()=>ownerIsCurrent()&&activeEditLockRef.current?.leaseOwnerId===claimedLock.leaseOwnerId&&lockCoordinator.current.isCurrent(claimedLock.generation)&&sameCloudConfig(getSupabaseConfig(),leaseConfig);
    try{
      if(saveTimer.current){window.clearTimeout(saveTimer.current);saveTimer.current=null;}
  ...
  const claimExclusiveItemLease=async(sectionKey:string,label:string,selectedKeys?:readonly string[]):Promise<AppData|null>=>{
    ...
    if(await claimEditingLock(sectionKey,label,undefined,true,liveAuthorized,selectedKeys)!=='owned')return null;
    return refreshAfterItemLease(sectionKey);
  };
  const releaseExclusiveItemLease=async(sectionKey:string)=>{
    const lock=activeEditLockRef.current;
    if(!lock)return true;
    if(lock.sectionKey!==sectionKey)return false;
    const handoff=relatedMutationHandoffInFlight.current;
    if(relatedMutationHandoffMatchesCurrent(lock)){
      if(handoff?.discardRejected&&!handoff.pending){if(!await handoff.discardRejected())return false;}
      else return releaseCurrentEditLock();
    }
  ...
  const operationalActiveVessels=useMemo(()=>activeVessels.map(vessel=>resolveVesselWithItineraryProjection(vessel,itineraryOperationalFeed.records[vessel.id],itineraryOperationalFeed.projectionNow)),[activeVessels,itineraryOperationalFeed.records,itineraryOperationalFeed.projectionNow]);
  ...
  const closedStatsTasks = useMemo(() => roleVisibleTasks.filter(t=>taskMatchesFilters(t,closedFilters,vesselMap,currentUser,false,canViewAllVessels,Boolean(currentUser&&t.ownerUserIds.includes(currentUser.id)))),[roleVisibleTasks,vesselMap,currentUser,closedFilters,canViewAllVessels]);

  const preserveMemberDraft=Boolean(memberEditor.current?.preservesDraft(editingTaskId)&&taskEditorAuthorizationEpoch===authorizationEpoch&&activeVessels.some(v=>v.id===memberEditor.current?.scope));
  if (!cloudBootstrapped&&!preserveMemberDraft) return <>{appVersionUpdateNotice}<div className="login-page"><div className="login-card loading-card"><h2>正在載入雲端主資料</h2><p className="muted">請稍候，完成前不會寫入或覆蓋資料。</p></div></div></>;
  ...
  const productionCloudSafetyGateBlocked=shouldRenderProductionCloudSafetyGate({
    productionCloudUnavailable:productionCloudUnavailable&&!preserveMemberDraft,
    editingVesselId,
  ...
  const navigateToTab = async (nextTab:Tab) => {
    if(nextTab!==tab&&trackingNavigationGuard.current&&!await trackingNavigationGuard.current())return;
    if(memberEditor.current)return; // Explicit editor close/discard owns its lifecycle.
  ...
  const openTaskEditor = async (task: TaskItem, vesselId:string, requestGeneration:number):Promise<TaskOpenResult> => {
    ...
    if(memberConfig&&memberAuthority?.source==='records-v1'&&usesPerVesselProgress(task)){
      ...
      if(!selected||!requestIsCurrent()){if(requestIsCurrent()&&editor.message)alert(editor.message);await editor.close();if(memberEditor.current===editor)memberEditor.current=null;return 'failed';}
  ...
  const changeTaskMemberScope=async(scope:string):Promise<TaskItem|null>=>{
    const member=memberEditor.current;if(!member||!member.isUsable())return null;
    const generation=++memberScopeGeneration.current;
    const scopeIsCurrent=()=>generation===memberScopeGeneration.current&&memberEditor.current===member&&member.isUsable();
    // Invalidate member reads/claims synchronously, including overall -> member ABA.
    ...
    if(scope==='overall'){
      ...
      if(!snapshot||!scopeIsCurrent())return null;
  ...
  const addTaskForVessel = async (vesselId: string, returnToVessel = false, returnToBatchManaged = false, batchContext?:BatchTaskReturnContext, entryIsCurrent:()=>boolean=()=>true):Promise<boolean> => {
    ...
    const openingIsCurrent=()=>entryIsCurrent()&&requestActor===liveCurrentUserId.current&&requestIdentityGeneration===identitySessionGeneration.current&&requestCloudIdentity===cloudConfigIdentity(getSupabaseConfig())&&authorizationEpoch===liveAuthorizationEpoch.current;
  ...
  const retainTrackingEditorLeases=async(lock:ActiveEditLock,leases:OwnedEditLockBundleLease[],config:ResolvedSupabaseConfig)=>{
    if(!relatedMutationLeaseMatches(lock,activeEditLockRef.current))return;
    const all=[...new Map([...(activeEditLockRef.current?.bundle||[]),...leases].map(lease=>[lease.sectionKey,lease])).values()];
    const seeds=[...new Set([...(lock.bundleSeeds||[lock.sectionKey]),...all.filter(lease=>lease.sectionKey.startsWith('tracking:')).map(lease=>lease.sectionKey)])];
    const keys=[...new Set(seeds.flatMap(key=>relatedEntityLockKeysForSection(liveData.current,key)))].sort();
    const requests=keys.filter(key=>!all.some(lease=>lease.sectionKey===key)).map(key=>({sectionKey:key,label:`跟蹤關聯｜${key}`,leaseOwnerId:`${lock.ownerUserId}:${crypto.randomUUID()}`}));
    for(const lease of all)leaseCloudConfigs.current.set(lease.leaseOwnerId,{sectionKey:lease.sectionKey,config});
    const current=()=>relatedMutationLeaseMatches(lock,activeEditLockRef.current)&&lockCoordinator.current.isCurrent(lock.generation)&&sameCloudConfig(config,getSupabaseConfig())&&liveAuthorizationEpoch.current===lock.authorizationEpoch;
    const result=await acquireEditLockBundle(requests,request=>{
      leaseCloudConfigs.current.set(request.leaseOwnerId,{sectionKey:request.sectionKey,config});
      return runCloudSaveQueueRpc('保留新建關聯編輯鎖',signal=>claimEditLock(request.sectionKey,request.leaseOwnerId,lock.ownerUserName,75,config,signal),8_000);
    },async request=>{await runCloudSaveQueueRpc('回滾新建關聯編輯鎖',signal=>releaseEditLock(request.sectionKey,request.leaseOwnerId,config,signal),8_000);leaseCloudConfigs.current.delete(request.leaseOwnerId);},current);
    if(result.status==='owned')all.push(...result.leases);
    if(!current())return;
    const latest=activeEditLockRef.current!;
    const owned={...latest,bundle:all,bundleSeeds:seeds,validatedUntilMs:Math.min(latest.validatedUntilMs,...leases.map(lease=>conservativeLeaseDeadline(lease.expiresAt)),...(result.status==='owned'?result.leases.map(lease=>conservativeLeaseDeadline(lease.expiresAt)):[]))};
    activeEditLockRef.current=owned;setActiveEditLock(owned);
    if(result.status!=='owned')freezeRelatedMutationDraft(owned);
  };
  const runDurableRelatedMutation=async(sectionKey:string,label:string,apply:()=>boolean,additionalLockKeys:(snapshot:AppData)=>readonly string[]=()=>[],onRecordPrepared?:(attempt:TrackingRecordAttempt)=>void):Promise<boolean>=>{
    ...
    const identityIsCurrent=()=>Boolean(
      ...
      &&lockCoordinator.current.isCurrent(expectedLease.generation)
      &&leaseCloudConfigs.current.get(expectedLease.leaseOwnerId)?.sectionKey===sectionKey
      &&sameCloudConfig(leaseCloudConfigs.current.get(expectedLease.leaseOwnerId)?.config,config)
    );
    const sessionIsCurrent=()=>identityIsCurrent()&&mutationLeaseIsOwned(sectionKey);
    ...
    const ownedEntryKeys=new Set((expectedLease.bundle||[]).map(lease=>lease.sectionKey));
    const requests=plannedLockKeys.filter(key=>key!==sectionKey&&!ownedEntryKeys.has(key)).map(key=>({sectionKey:key,label:`${label}｜${key}`,leaseOwnerId:uid('related-lease')}));
    ...
    const mutationHandoff:RelatedMutationEditorHandoff=createDurableRelatedMutationHandoff(expectedLease,identityIsCurrent,label);
    ...
    try{
      ...
      if(saveTimer.current){window.clearTimeout(saveTimer.current);saveTimer.current=null;}
      assertBundleActive();
      await enqueueCloudSave(liveData.current,identityIsCurrent,true,sessionIsCurrent,undefined,undefined,onRecordPrepared);
      if(!identityIsCurrent())throw new StaleAsyncConfigError();
      ...
      if(sectionKey.startsWith('tracking:')){
        await stopHeartbeat();
        await retainTrackingEditorLeases(expectedLease,result.leases,config);
        clearGuards();
        return true;
      }
      ...
      // Cancel/Close may dispose a proven rejected operation, never an unknown one.
      if(applied&&identityIsCurrent()&&error instanceof CloudBlockPatchRejectedError){
        const rejectedSnapshot=liveData.current;
        mutationHandoff.discardRejected=async()=>{
          if(!identityIsCurrent()||!appDataContentEqual(liveData.current,rejectedSnapshot))return false;
          const remote=await fetchCloudData(config);
          if(!remote||!identityIsCurrent()||!appDataContentEqual(liveData.current,rejectedSnapshot))return false;
          assertRemoteExtendsDurableHistory(cloudIdentity(config),confirmedCloudData.current!,remote);
          await releaseRelated();
      ...
      if(!applied&&relatedMutationHandoffInFlight.current===mutationHandoff)relatedMutationHandoffInFlight.current=null;
  ...
  const loadTrackingScope=async(vesselId:string,ids:string[]=[]):Promise<AppData|null>=>{
    const owner=trackingIdentity();
    const allowed=()=>{const snapshot=liveData.current,actor=snapshot.users.find(u=>u.id===liveCurrentUserId.current&&u.isActive),vessel=snapshot.vessels.find(v=>v.id===vesselId&&v.isActive);return Boolean(owner===trackingIdentity()&&actor&&actor.role!=='vessel'&&vessel&&canAccessAllVessels(snapshot.settings.rolePermissions,actor,[vessel]));};
  ...
  const claimTrackingEditor=async(vesselId:string,ids:string[]):Promise<AppData|null>=>{
    ...
    const owner=trackingIdentity();
    const snapshot=await claimExclusiveItemLease(`tracking:${ids[0]}`,'配件／物料／工程跟蹤',ids.map(id=>`tracking:${id}`));
    return owner===trackingIdentity()&&snapshot?.trackingItems?.filter(item=>ids.includes(item.id)).every(item=>item.vesselId===vesselId)?snapshot:null;
  };
  const trackingEditorIsWritable=(ids:readonly string[])=>{
    const lock=activeEditLockRef.current;
    return Boolean(lock&&lock.sectionKey.startsWith('tracking:')&&mutationLeaseIsOwned(lock.sectionKey)&&ids.every(id=>lock.bundle?.some(lease=>lease.sectionKey===`tracking:${id}`)));
  };
  const reconcileTrackingReceipt=async(submission:TrackingSubmission,record:TrackingRecordAttempt):Promise<boolean>=>{
    ...
    const check=()=>{if(!current())throw new StaleAsyncConfigError();};
    const lookup=(id:string)=>runCloudSaveQueueRpc('確認原跟蹤提交',signal=>getCloudBlockPatchReceipt(id,record.operations,record.savedBy,record.actorId,record.actorGuard,record.authorizationGuard,record.guards,commandConfig,signal),8_000);
    try{
      if(record.authority){const live=await readBrowserAuthority(config);check();if(!sameAuthority(record.authority,live))return false;}
      let receipt=await lookup(record.operationId);check();
      if(receipt.status==='missing'){
        const primary=trackingAttempt.current?.sectionKey;
        if(!primary||!mutationLeaseIsOwned(primary))return false;
        receipt=await runCloudBlockPatchWithReceipt({operationId:record.operationId,assertCurrent:check,lookup,shouldReconcile:error=>!(error instanceof CloudBlockPatchRejectedError||error instanceof CloudBlockPatchConflictError||error instanceof StaleAsyncConfigError),submit:async id=>{check();if(!mutationLeaseIsOwned(primary))throw new StaleAsyncConfigError();if(record.authority)await assertAuthorityAdmission(config,record.authority);check();return runCloudSaveQueueRpc('重試原跟蹤提交',signal=>applyCloudBlockPatchV2(id,record.operations,record.savedBy,record.actorId,record.actorGuard,record.authorizationGuard,record.guards,commandConfig,signal),12_000);}});
      }
      check();const remote=await fetchCloudDataRpc(commandConfig,undefined,undefined,record.scope);check();
      if(!remote||remote.revision<receipt.revision)throw new CloudBlockPatchConfirmedRefreshError(receipt);
      assertRemoteExtendsDurableHistory(activeCloudIdentity.current,null,remote,record.authority);
      ...
      await Promise.allSettled(record.guards.filter(guard=>guard.section_key!==activeEditLockRef.current?.sectionKey&&!activeEditLockRef.current?.bundle?.some(lease=>lease.sectionKey===guard.section_key&&lease.leaseOwnerId===guard.locked_by)).map(guard=>runCloudSaveQueueRpc('釋放原跟蹤關聯鎖',signal=>releaseEditLock(guard.section_key,guard.locked_by,config,signal),8_000)));
      for(const [key,value] of transientCloudBlockLockGuards.current)if(record.guards.some(guard=>guard.section_key===value.guard.section_key&&guard.locked_by===value.guard.locked_by))transientCloudBlockLockGuards.current.delete(key);
      check();
      if(activeEditLockRef.current?.sectionKey.startsWith('tracking:'))await retainTrackingEditorLeases(activeEditLockRef.current,[],config);
      localStorage.removeItem(trackingReceiptKey(owner,submission.context.operationId));if(trackingAttempt.current)trackingAttempt.current.confirmed=true;return true;
    }catch(error){if(current())setCloudStatus(`原提交尚未安全確認；精確資料及草稿保留：${cloudErrorMessage(error)}`);return false;}
  };
  const submitTracking=async(submission:TrackingSubmission):Promise<boolean>=>{
    const owner=submission.identity;
    const isCurrent=()=>owner===trackingIdentity()&&submission.context.actorId===liveCurrentUserId.current;
    if(!isCurrent()||!getSupabaseConfig()){alert('跟蹤操作需要同一登入工作階段及雲端確認；原草稿保留。');return false;}
    const previous=trackingAttempt.current;
    const stored=localStorage.getItem(trackingReceiptKey(owner,submission.context.operationId));
    if(stored){try{return await reconcileTrackingReceipt(submission,JSON.parse(stored) as TrackingRecordAttempt);}catch{return false;}}
    if(previous?.submission.context.operationId===submission.context.operationId){
      if(JSON.stringify(previous.submission)!==JSON.stringify(submission))return false;
      if(previous.confirmed)return true;
      if(previous.applied){
        setCloudStatus('原提交尚未確認且缺少精確請求；草稿保留，禁止重建或更換 operation。');return false;
      }
    }
    if(previous?.applied&&!previous.confirmed){alert('前一筆追蹤提交尚未確認，不可建立另一筆。');return false;}
    const command=structuredClone(submission.command);
    const ids=command.type==='lifecycle'?command.targets.map(target=>target.id):command.items.map(item=>item.id);
    if(!ids.length||ids.length>100||new Set(ids).size!==ids.length)return false;
    const editor=activeEditLockRef.current;
    const admitted=Boolean(editor?.sectionKey.startsWith('tracking:')&&ids.every(id=>editor.bundleSeeds?.includes(`tracking:${id}`)));
    const primary=admitted?editor!.sectionKey:`tracking:${ids[0]}`;
    const attempt={submission:structuredClone(submission),sectionKey:primary,applied:false,confirmed:false};
    trackingAttempt.current=attempt;
    try {
      if(command.type==='create'){
        const first=command.items[0];
        trackingCreationAdmissions.current.set(primary,{actorId:submission.context.actorId,vesselId:first.vesselId,identity:owner});
      }
      // Admission for an absent, reserved source is explicit, not projected existence.
      if(admitted){if(!requireMutationLease(primary))return false;}
      else if(!await claimExclusiveItemLease(primary,'配件／物料／工程跟蹤',command.type==='create'?undefined:ids.map(id=>`tracking:${id}`))||!isCurrent())return false;
      const keys=(snapshot:AppData)=>[
        ...ids.flatMap(id=>relatedEntityLockKeysForSection(snapshot,`tracking:${id}`)),
        ...(command.type==='sync'?command.items.map(input=>internalControlCreationLockKey(input.item.id)):[]),
      ];
      const apply=()=>{
        if(!isCurrent())return false;
        let accepted=false;
        flushSync(()=>setData(prev=>{
          ...
          try {
            const next=runTrackingUiCommand(prev,command,submission.context);
            const actor=prev.users.find(user=>user.id===submission.context.actorId&&user.isActive)!;
            accepted=true;attempt.applied=true;
            return withAudit(next,actor,`跟蹤${command.type==='lifecycle'?command.action:command.type}`,'tracking',ids.join(','),`精確來源 ${ids.join(',')}｜來源操作 ${submission.context.operationId}`);
          }catch(error){alert(error instanceof Error?error.message:String(error));return prev;}
        }));
        return accepted;
      };
      const confirmed=await runDurableRelatedMutation(primary,'跟蹤保存',apply,keys,record=>{if(!isCurrent())throw new StaleAsyncConfigError();localStorage.setItem(trackingReceiptKey(owner,submission.context.operationId),JSON.stringify(record));});
      if(confirmed&&isCurrent()){localStorage.removeItem(trackingReceiptKey(owner,submission.context.operationId));attempt.confirmed=true;return true;}
      if(!attempt.applied&&!admitted){await releaseExclusiveItemLease(primary);trackingCreationAdmissions.current.delete(primary);}
      return false;
    }catch(error){if(isCurrent())alert(error instanceof Error?error.message:String(error));return false;}
  };
  const discardTrackingRejected=async()=>{
    const attempt=trackingAttempt.current;
    if(!attempt||attempt.submission.identity!==trackingIdentity())return false;
    if(attempt.applied){const handoff=relatedMutationHandoffInFlight.current;if(!handoff?.discardRejected||handoff.pending||!await handoff.discardRejected())return false;}
    if(!await releaseExclusiveItemLease(attempt.sectionKey))return false;
    trackingCreationAdmissions.current.delete(attempt.sectionKey);localStorage.removeItem(trackingReceiptKey(attempt.submission.identity,attempt.submission.context.operationId));trackingAttempt.current=null;return true;
  };
  const releaseTracking=async()=>{
    const attempt=trackingAttempt.current;
    if(!attempt){const lock=activeEditLockRef.current;return lock?.sectionKey.startsWith('tracking:')?releaseExclusiveItemLease(lock.sectionKey):true;}
    if(attempt.submission.identity!==trackingIdentity())return false;
    if(!await releaseExclusiveItemLease(attempt.sectionKey))return false;
    trackingCreationAdmissions.current.delete(attempt.sectionKey);
  ...
  const saveTask = async (candidate: TaskItem, creating: boolean, expectedUpdatedAt: string, expectedRevision: number, pendingRun?:PendingCreationRunContext) => {
    ...
    const applyTaskSave=(prev:AppData):AppData=>{
      ...
      if(saved.ownerUserIds.some(id=>!( !creating&&savedScopeIds.every(vesselId=>taskVesselIds(previous).includes(vesselId))&&canRetainVesselCommonContact(previous,id,previous.ownerUserIds,prev.users.find(user=>user.id===id)))&&!isEligibleTaskOwner(prev.settings.rolePermissions,prev.users.find(user=>user.id===id),savedScopeVessels))){failure='負責人已停用或不具備最終涉船範圍權限，請重新選擇';return prev;}
      ...
      rebindReopenedVesselResponsibilities(prev,draft);
  ...
  const listBatchConfig=getSupabaseConfig();
  const listBatchConfigToken=listBatchConfig?configIoCoordinator.current.begin(listBatchConfig):null;
  const listBatchActorId=currentUser.id;
  const listBatchSession=identitySessionGeneration.current;
  const listBatchViewVersion=listBatchViewGeneration.current;
  const listBatchContext={
    identity:JSON.stringify([listBatchActorId,authorizationEpoch,listBatchSession,cloudConfigIdentity(listBatchConfig),listBatchConfigToken?.epoch,tab,listBatchViewVersion,data.settings.rolePermissions]),
    isCurrent:()=>liveCurrentUserId.current===listBatchActorId&&identitySessionGeneration.current===listBatchSession&&liveAuthorizationEpoch.current===authorizationEpoch&&liveListBatchView.current===tab&&listBatchViewGeneration.current===listBatchViewVersion&&(listBatchConfigToken?configIoCoordinator.current.isCurrent(listBatchConfigToken,getSupabaseConfig()):!getSupabaseConfig()),
  };
  const saveTaskWithListFeedback=async(candidate:TaskItem,creating:boolean,expectedUpdatedAt:string,expectedRevision:number)=>{
    const fromTotalList=creating&&tab==='total';
    const saved=await saveTask(candidate,creating,expectedUpdatedAt,expectedRevision);
    if(saved&&fromTotalList&&listBatchContext.isCurrent()){
      // A queued creation also returns true to close its editor. Only a confirmed
      // record may produce success feedback; never replace its waiting warning.
      const snapshot=getSupabaseConfig()?confirmedCloudData.current:liveData.current;
      const confirmed=snapshot?.tasks.find(task=>task.id===candidate.id);
      if(snapshot&&confirmed){
        const vessels=Object.fromEntries(snapshot.vessels.map(vessel=>[vessel.id,vessel]));
        if(!taskMatchesFilters(confirmed,filters,vessels,currentUser,true,canViewAllVessels,confirmed.ownerUserIds.includes(currentUser.id)))showSaveToast('success','新增要事已保存','新要事不符合目前的清單篩選條件；可清除篩選後查看。',10_000);
      }
    }
    return saved;
  };
  const runTaskMutationWithLockBundle=async(taskIds:string[],label:string,mutation:(fresh:AppData)=>boolean,additionalLockKeys:(snapshot:AppData)=>readonly string[]=()=>[]):Promise<boolean>=>{
    ...
    const entry=activeEditLockRef.current;
    if(entry?.bundle&&entry.status==='owned'&&entry.ownerUserId===actorId&&entry.authorizationEpoch===expectedAuthorizationEpoch&&plannedLockKeys.every(key=>entry.bundle!.some(lease=>lease.sectionKey===key))){
      return runDurableRelatedMutation(entry.sectionKey,label,()=>mutation(liveData.current),()=>plannedLockKeys);
    }
  ...
  const leaveCurrentIdentity = async () => {
    if(trackingNavigationGuard.current&&!await trackingNavigationGuard.current())return;
  ...
  const retainedRelatedTask=activeEditLock?.sectionKey===`task:${editingTaskId}`&&(activeEditLock.bundle||relatedMutationHandoffMatchesCurrent(activeEditLock))
  ...
  return <div className="app">
    <header className="topbar no-print"><div className="topbar-inner">
      ...
      <nav className="nav topbar-primary-nav">
        {([['dashboard','船隊看板'],['morning','早會工作台'],['meeting','臨會/專題'],['work',`我的待辦${myWorkTaskCount?`（${myWorkTaskCount}）`:''}`],['total',currentUser.role==='vessel'?'本船待辦':'待辦總表'],['closed','已結案'],['internalControl','內控異常'],['tracking','配件/物料/工程跟蹤'],['reports','報告中心'],['stats','數據分析'],['management','管理']] as [Tab,string][]).filter(([k])=>canAccessTab(currentUser, k)&&(k!=='reports'||canExportReports)&&(k!=='management'||canEnterManagement)).map(([k,label]) => <button key={k} className={`${tab===k?'active':''} ${tab!==k&&['dashboard','work','internalControl'].includes(k)?'gradient-nav-label':''}`.trim()} onClick={() => { if (!canAccessTab(currentUser,k)) return; if (k==='reports' && !canExportReports) return alert('目前角色未獲授權預覽或匯出報告'); if (k==='management' && !requireManage()) return; navigateToTab(k); }}>{label}</button>)}
      </nav>
    ...
    <main className="container">
      ...
      {tab==='total' && <ListPanel title={currentUser.role==='vessel'?'本船待辦清單':'總清單'} tasks={filteredTasks} statsTasks={statsTasks} data={roleVisibleData} visibleVessels={activeVessels} filters={filters} setFilters={setFilters} fleetTags={fleetTags} userMap={userMap} exportedBy={currentUser.name} onCreateTask={canCreateTasks&&currentUser.role!=='vessel'?(vesselId,isCurrent)=>addTaskForVessel(vesselId,false,false,undefined,isCurrent):undefined} onEdit={openTask} onPrint={() => print('船舶記事總清單')} batchContext={listBatchContext} onBatchComplete={batchCompleteTasks} onBatchDelete={batchDeleteTasks} canEdit={canEditBusinessContent&&currentUser.role!=='vessel'} canPrint={canExportReports} canComplete={canCloseTasks&&currentUser.role!=='vessel'} canDelete={canDeleteTasks} />}
      {tab==='work' && <WorkCenter
        data={roleVisibleData}
        user={currentUser}
        vessels={activeVessels}
        onOpenTask={openTask}
        onOpenInternalControl={caseId=>{if(canAccessTab(currentUser,'internalControl'))setRequestedInternalControlCaseId(caseId);}}
...
function ReportCenter({ data, visibleVessels, user, selected, setSelected, canSaveDailyMorning, onSaveDailyMorning, onOpenPreview, onOpenHistory, onPrint }: { data:AppData; visibleVessels:Vessel[]; user:UserAccount; selected:string[]; setSelected:(ids:string[])=>void; canSaveDailyMorning:boolean; onSaveDailyMorning:(at:string)=>Promise<boolean>; onOpenPreview:()=>void; onOpenHistory:(report:AgendaReport)=>void; onPrint:()=>void }) {
  ...
  const [itineraryHistoryRefreshToken,setItineraryHistoryRefreshToken]=useState(0);
...
export function ListPanel({ title, tasks:incomingTasks, statsTasks=incomingTasks, batchContext, data, visibleVessels, filters, setFilters, fleetTags, userMap, exportedBy, onCreateTask, onEdit, onPrint, onBatchComplete, onBatchDelete, canEdit, canPrint, canComplete, canDelete }: { title:string; tasks:TaskItem[]; statsTasks?:TaskItem[]; data:AppData; visibleVessels:Vessel[]; filters:FilterState; setFilters:(f:FilterState)=>void; fleetTags:string[]; userMap:Record<string,UserAccount>; exportedBy:string; onCreateTask?:OpenTaskCreation; onEdit:(t:TaskItem)=>void; onPrint:()=>void; onBatchComplete:(ids:string[])=>boolean|Promise<boolean>; onBatchDelete:(ids:string[])=>boolean|Promise<boolean>; canEdit:boolean; canPrint:boolean; canComplete:boolean; canDelete:boolean; batchContext:{identity:string;isCurrent:()=>boolean} }) {
  ...
  const batchSelection=useRef<{context:string;tasks:TaskItem[];pending:boolean}|null>(null);
  const contextKey=JSON.stringify([batchContext.identity,filters,visibleVessels.map(v=>v.id).sort(),canComplete,canDelete]);
  ...
  useEffect(()=>{setSelectedIds([]);},[batchContext.identity]);
  // Read projection only: an optimistic close/delete is not a committed ACK.
  // Context changes discard it; writes still revalidate the live App/SQL rows.
  const retained=batchContext.isCurrent()&&batchSelection.current?.context===contextKey?batchSelection.current.tasks.filter(t=>selectedIds.includes(t.id)):[];
  const retainedIds=new Set(retained.map(t=>t.id));
  const tasks=[...incomingTasks.filter(t=>!retainedIds.has(t.id)),...retained];
  ...
  useEffect(()=>{setSelectedIds(previous=>{const next=sanitizeTaskSelection(previous,tasks);return next.length===previous.length?previous:next;});setPage(1);},[taskIdsKey]);
  ...
  const toggleAll=()=>{selectionVersion.current+=1;setSelectedIds(previous=>allSelected?previous.filter(id=>!pagedTasks.items.some(task=>task.id===id)):Array.from(new Set([...previous,...pagedTasks.items.map(task=>task.id)])));};
  ...
  const runSelected=async(ids:string[],allowed:boolean,submit:(ids:string[])=>boolean|Promise<boolean>)=>{
    if(!allowed||!ids.length||!batchContext.isCurrent()||batchSelection.current?.pending)return;
    const selection={context:contextKey,tasks:[...selectedTasks],pending:true};
    const version=selectionVersion.current;
    batchSelection.current=selection;
    setSelectedIds(previous=>[...previous]);
    try{
      const committed=await submit(ids);
      ...
      if(committed){batchSelection.current=null;if(selectionVersion.current===version)setSelectedIds([]);else setSelectedIds(previous=>[...previous]);}
    }catch{
      // A rejected/unknown callback is not permission to dispose this selection.
