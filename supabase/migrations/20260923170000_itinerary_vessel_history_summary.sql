-- Upgrade the existing read-only history list with compact frozen first/second rows.
-- No business-data writes, new grants, scheduler changes, or save-path changes.
-- Install after 20260923160000_itinerary_vessel_history.sql and before the matching frontend.
begin;
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'overviewRows',coalesce((
      select jsonb_agg(jsonb_build_object(
        'previousPortName',h.item -> 'previousPortName',
        'currentVesselState',h.item -> 'currentVesselState',
        'voyageNumber',h.item -> 'voyageNumber','portDockName',h.item -> 'portDockName',
        'etaUtc',h.item -> 'etaUtc','etaTimeZone',h.item -> 'etaTimeZone',
        'etbUtc',h.item -> 'etbUtc','etbTimeZone',h.item -> 'etbTimeZone',
        'etdUtc',h.item -> 'etdUtc','etdTimeZone',h.item -> 'etdTimeZone',
        'portTimeZone',h.item -> 'portTimeZone'
      ) order by h.sort_order,h.row_id,h.position)
      from (
        -- Always the saved display order. Never advance by now()/ETD or scan row 3.
        select e.item,coalesce((e.item ->> 'sortOrder')::numeric,0) sort_order,
          coalesce(e.item ->> 'rowId','') row_id,e.position
        from jsonb_array_elements(v.item -> 'rows') with ordinality e(item,position)
        order by sort_order,row_id,position limit 2
      ) h
    ),'[]'::jsonb)
