...
create or replace function public.sd_itinerary_utc_offset_valid(p_value text)
returns boolean
language plpgsql
immutable
...
declare
  ...
  v_total integer;
begin
  if p_value = 'UTC' then return true; end if;
  v_match := regexp_match(p_value, '^UTC([+-])([0-9]{1,2})(?::(00|15|30|45))?$');
  if v_match is null then return false; end if;
  v_hours := v_match[2]::integer;
  v_minutes := coalesce(v_match[3], '0')::integer;
  v_total := v_hours * 60 + v_minutes;
  if v_match[1] = '-' then v_total := -v_total; end if;
  return v_total between -720 and 840;
...
revoke all on function public.sd_itinerary_utc_offset_valid(text) from public, anon, authenticated;
revoke all on function public.sd_itinerary_rows_valid(jsonb) from public, anon, authenticated;

comment on function public.sd_itinerary_utc_offset_valid(text) is 'Validates fixed UTC offsets from UTC-12 through UTC+14 in 15-minute increments.';
