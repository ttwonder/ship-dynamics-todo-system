...
begin
  ...
  for v_row, v_ordinal in select value, ordinality from jsonb_array_elements(p_rows) with ordinality loop
    ...
    if (not v_is_v2 and (select count(*) from jsonb_object_keys(v_row)) <> cardinality(v_base_allowed) + v_notes_count)
       or (v_is_v2 and (select count(*) from jsonb_object_keys(v_row)) <> cardinality(v_base_allowed) + cardinality(v_v2_allowed) + v_notes_count) then return false; end if;
...
comment on function public.sd_itinerary_rows_valid(jsonb) is 'Validates legacy v1 and calculation-v2 itinerary rows, with optional bounded notesText, canonical Purpose, per-LT UTC offsets, calculation anchor, and fractional durations.';
