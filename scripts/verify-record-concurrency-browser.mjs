...
receipt.inputs=Object.fromEntries(['scripts/verify-record-concurrency-browser.mjs','scripts/record-storage-native-qa.mjs','scripts/record-storage-local-qa.mjs','src/App.tsx'].map(p=>[p,hash(fs.readFileSync(p,'utf8'))]));
...
try{
 ...
 assert.equal(/可以安全關閉|沒有未保存修改|沒有尚未保存的修改/.test(await b.text()),false,'D1 child-only unsent draft must not advertise safe close after sync');
 assert.equal(receipt.network.filter(r=>r.caseId==='U1'&&r.actor==='qa-operator'&&r.rpc===patchRpc).length,0,'D1 unsent B has zero business writes');
 receipt.cases.push({caseId:'D1',layer:'real-ui-native-sql',status:'PASS',sameDraftNode:true,unsentBusinessWrites:0});
 await b.click('立即保存');
 await until(()=>b.eval("Boolean(document.querySelector('.save-status-strip.dirty'))"),'D2 no-delta Save keeps draft feedback');
 assert.equal(/可以安全關閉|沒有未保存修改|沒有尚未保存的修改/.test(await b.text()),false,'D2 header Save must not certify child draft');
 assert.equal(await b.eval(`window.__draftNode===${field}&&window.__draftNode.value==='U1 B UNSAVED THEN SAVED'`),true);
 assert.deepEqual((await locks()).map(l=>l.section_key),['vessel:qa-v1']);
 assert.equal(receipt.network.filter(r=>r.caseId==='U1'&&r.actor==='qa-operator'&&r.rpc===patchRpc).length,0);
 receipt.cases.push({caseId:'D2',layer:'real-ui-native-sql',status:'PASS',unsentBusinessWrites:0});save();
 ...
 assert.equal(receipt.network.filter(r=>r.caseId==='D3'&&r.rpc===patchRpc).length,0);
 receipt.cases.push({caseId:'D3',layer:'real-ui-native-sql',status:'PASS',cancelZeroWrites:true});
 currentCase='D4';await b.sync();assert.ok((await b.text()).includes('本頁沒有未保存修改，現在可以安全關閉或重新整理。'),'D4 clean sync still succeeds');
 await b.click('立即保存');await until(async()=>(await b.text()).includes('沒有尚未保存的修改，現在可以安全關閉或重新整理頁面。'),'D4 clean no-delta Save success');
