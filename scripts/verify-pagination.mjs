...
try{
  ...
  assert.match(app,/paginateItems\(sortedTasks,\s*page\)/,'待办总清单与已结案清单必须在排序后使用分页结果');
  assert.match(app,/pagedTasks\.items\.map/,'待办清单不得继续渲染全部结果');
  assert.match(meetings,/paginateItems\(orderedMeetings,\s*meetingPage\)/,'临会清单必须在列表排序后使用分页结果');
