...
try{
 ...
 const items=Array.from({length:65},(_,i)=>({id:String(i).padStart(3,'0'),kind:'supply',vesselId:'a',referenceNo:'REF-'+String(65-i).padStart(3,'0'),applicationDate:'2026-09-01',description:'test',urgency:i%2?'urgent':'normal',purchaseNos:i===0?'':i%2?'A':'B',progress:'',expectedDate:'',supplementalNotes:'',deliveryStatus:'not-delivered',isClosed:false,createdAt:'2026-09-01T00:00:00Z',updatedAt:'2026-09-01T00:00:00Z',statusLogs:[]}));
 ...
 for(const direction of ['asc','desc'])assert.equal(selectTrackingRows(items,{...q,sort:{key:'purchaseNos',direction}}).at(-1).id,'000');
 assert.equal(selectTrackingRows(items,{...q,filters:{purchaseNos:{values:['A','B']},urgent:{values:['是']}}}).length,32);
 assert.equal(selectTrackingRows(items,{...q,filters:{purchaseNos:{mode:'blank'}}}).length,1);
 ...
 assert.equal(selectTrackingRows([closed,delivered,cancelled],{...q,tab:'engineering-closed'}).length,0);assert.equal(selectTrackingRows([closed,delivered,cancelled],{...q,tab:'engineering-open'}).length,1);
