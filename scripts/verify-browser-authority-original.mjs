import {primary} from './browser-authority-original-scenario.mjs';
...
async function makePage(actor,context){
 ...
 p.screen=async name=>{receipt.screenshotsSuppressed??=[];receipt.screenshotsSuppressed.push({name,reason:'privacy; DOM proof only'});};
...
async function freshReadback(name,expected){
 ...
 assert.ok(isDeepStrictEqual(actual,expected),'complete new SQL connection readback');
 ...
 for(const p of actors.filter(p=>!p.reader)){
  ...
  assert.ok(isDeepStrictEqual(model,JSON.parse(JSON.stringify(normalizeAppData(actual.payload)))),'new document complete original cloud read');
  stoppingSessions.add(fresh.s);await call('Fetch.disable',{},fresh.s);await call('Target.closeTarget',{targetId:fresh.targetId});
 }
 fs.writeFileSync(path.join(run,name+'-readback.json'),JSON.stringify({hash:hash(actual),readbackPersisted:false},null,2));
}
...
try{
 ...
 ws.addEventListener('message',event=>{const m=JSON.parse(event.data);if(m.id){const p=pending.get(m.id);if(p){pending.delete(m.id);m.error?p.reject(new Error(m.error.message)):p.resolve(m.result);}return;}
  const handle=async()=>{
   ...
   if(m.method==='Page.javascriptDialogOpening'){
    const {type,message}=m.params;receipt.dialogs??=[];receipt.dialogs.push({messageHash:hash(message),type});const privatePrompt=message==='尚有未提交的管理表單修改。確定放棄這些修改並繼續？';const accept=privatePrompt||type==='beforeunload'?privateDialogAccept:type==='confirm'&&['清除「','確定停用「','同步最新會保留本機修改','請盡量以船端修改為主','確定批量完成所選','確定結案會議','確定重新開啟會議','確定重新開啟此待辦'].some(t=>message.startsWith(t));
    if(!(currentCase==='P02-draft-pause-rejection'&&type==='alert')&&!accept&&!privatePrompt&&type!=='beforeunload'&&message!=='進站密碼已更新'&&!(currentCase==='MG-PRIVATE-GENERIC-ACK'&&message==='個人密碼已更新；下次登入需使用新密碼。'))receipt.errors.push('unexpected dialog: '+message);await call('Page.handleJavaScriptDialog',{accept},m.sessionId);
   }
   if(m.method==='Fetch.requestPaused'){
    ...
    if(allowed&&[patchRpc,'apply_ship_dynamics_block_patch_v2','save_ship_dynamics_task_member_v1','get_ship_dynamics_record_receipt_v1','get_ship_dynamics_block_patch_receipt'].some(name=>u.pathname.endsWith('/rpc/'+name))){outgoing.push({caseId:currentCase,rpc:u.pathname.split('/').at(-1),body:JSON.parse(m.params.request.postData),captured:Date.now()});fs.writeFileSync(path.join(run,'outgoing-before-sql.json'),JSON.stringify(outgoing.map(r=>({caseId:r.caseId,captured:r.captured,payloadHash:hash(r.body),operationId:r.body.p_operation_id})),null,2));}
   ...
   if(m.method==='Network.requestWillBeSent'&&m.params.request.url.startsWith(qa.origin+'/rest/v1/rpc/')){
    const b=JSON.parse(m.params.request.postData||'{}'),row={caseId:currentCase,actor:actors.find(p=>p.s===m.sessionId)?.actor,rpc:m.params.request.url.split('/').at(-1),operationId:b.p_operation_id,readScope:b.p_scope,targets:b.p_targets,handoverReadRevisionPresent:Boolean(b.p_authorization_guard&&Object.hasOwn(b.p_authorization_guard,'handoverReadRevision')),payloadHash:hash(b),started:m.params.wallTime*1000};
   ...
   if(m.method==='Network.loadingFinished'&&row){const response=await call('Network.getResponseBody',{requestId:m.params.requestId},m.sessionId);const v=JSON.parse(response.base64Encoded?Buffer.from(response.body,'base64').toString():response.body);Object.assign(row,{containsUnloadedDetail:JSON.stringify(v).includes('QA_UNLOADED_DETAIL_SENTINEL'),containsOtherMeetingHistory:JSON.stringify(v).includes('QA_OTHER_MEETING_HISTORY'),finished:Date.now(),result:v?.code??(v?.ok===true?'SQL_OK':'READ_OK'),ok:v?.ok,receiptStatus:v?.status,code:v?.code,message:v?.code==='55000'?v.message:undefined,conflictKey:v?.conflict_key,revision:v?.revision});save();}
 ...
 await login(a);
 const write=(name,v)=>fs.writeFileSync(path.join(run,name+'.json'),JSON.stringify(v,null,2));
 await primary({a,qa,native,call,until,wait,read,locks,field,receipt,save,hash,write,freshReadback,outgoing,setCase:value=>{currentCase=value;receipt.currentCase=value;save();}});
 receipt.status=receipt.status==='RUNNING'?'PASS':receipt.status;
}catch(e){failure=e;receipt.status='FAIL';receipt.failure={caseId:currentCase,code:e.code,message:String(e.message).split('\n')[0],stackFrames:e.stack?.split('\n').filter(x=>/^\s+at /.test(x)).slice(0,5)};for(const p of actors.filter(p=>!p.reader)){try{receipt['failureTextHash-'+p.actor]=hash(await p.text());await p.screen('failure-'+p.actor);}catch{}}console.error(JSON.stringify({status:'FAIL',caseId:currentCase,error:String(e.message).split('\n')[0],run}));}
finally{
