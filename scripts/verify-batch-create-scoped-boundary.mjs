...
const base='d1745dec677fb07d04a200d23ce3bc2326b1496d';
...
for(const path of paths){
 ...
 let expected=before;if(path==='src/App.tsx'){for(const [old,replacement] of JSON.parse(fs.readFileSync('scripts/record-batch-create-source-allowlist.json','utf8'))){assert.equal(expected.split(old).length,2,'exact unique allowed span');expected=expected.replace(old,replacement);}}assert.equal(now,expected,'precise source/UI/style/SQL/public/root '+path);
...
console.log(JSON.stringify({gate:'exact allowed internal App logic/prop delta; remaining original source/UI/SQL unchanged',base,checkedPaths:paths.length,jsxFiles,jsxBlocks,allowedProductChanges:['src/App.tsx: addTaskForVessel, refreshAfterItemLease, openBatchManagedVessels exact internal spans only; JSX unchanged']}));
