...
try {
  ...
  const task = (id, extra = {}) => ({
    id, vesselId: 'v1', vesselIds: ['v1'], sourceType: 'morning', priority: '中', isInternalControl: false, isClosed: false,
    createdAt: '2026-08-02T01:00:00.000Z', updatedAt: '2026-08-02T01:00:00.000Z', ...extra,
  });
  const internalCase = (id, extra = {}) => ({
    id, vesselId: 'v1', priority: '中', isClosed: false, createdAt: '2026-08-02T01:00:00.000Z', updatedAt: '2026-08-02T01:00:00.000Z', ...extra,
  });
  const result = classifyMorningAgenda({
    tasks: [
      task('history-task'),
      task('changed-task', { updatedAt: '2026-08-03T03:00:00.000Z' }),
    ...
    internalControlCases: [
      internalCase('linked-case', { updatedAt: '2026-08-03T05:00:00.000Z' }),
  ...
  assert.deepEqual(result.historyTasks.map(item => item.id), ['history-task']);
  assert.deepEqual(result.todayInternalControlCases.map(item => item.id).sort(), ['closed-case', 'linked-case']);
  assert.deepEqual(result.historyInternalControlCases.map(item => item.id), ['history-case']);
