...
export async function componentChecks({qa,call,evaluate,click,nodeClick,fill,until,text,screen,check}){
 ...
 await check('component-65-row-sort-before-page-selection-filter-and-hidden-filter',async()=>{
   ...
   await click('項目編號 ↕');assert.equal((await rowIds())[0],'r64');
   ...
   await click('選取全部符合條件 65 項');assert.ok((await text()).includes('已選 65 項'));await click('項目編號 ↑');assert.ok((await text()).includes('已選 65 項'));
   await nodeClick("[...document.querySelectorAll('summary')].find(n=>n.innerText==='全部欄位篩選')");await fill('[aria-label="供應商包含"]','A');assert.ok((await text()).includes('已選 0 項'));assert.ok((await text()).includes('已清除原選取'));
   await nodeClick("[...document.querySelectorAll('summary')].find(n=>n.innerText==='欄位設定')");
   await nodeClick("[...document.querySelectorAll('.tracking-preferences label')].find(n=>n.innerText==='供應商').querySelector('input')");
   assert.ok(await evaluate("document.querySelector('[aria-label=有效篩選]').innerText.includes('供應商（隱藏欄）')"));
 ...
 await check('component-column-preferences-actor-scope-keyboard-width-and-reset',async()=>{
   ...
   await evaluate("document.querySelector('[aria-label=調整項目編號欄寬]').focus()");await call('Input.dispatchKeyEvent',{type:'keyDown',key:'ArrowRight',code:'ArrowRight'});await call('Input.dispatchKeyEvent',{type:'keyUp',key:'ArrowRight',code:'ArrowRight'});
   assert.ok(await evaluate("document.querySelector('th.tracking-reference').getBoundingClientRect().width")>first);
   await evaluate("window.__trackingQA.change({actorId:'component-b',identity:'session-b'})");await until(()=>evaluate("Boolean([...document.querySelectorAll('.tracking-sort')].find(n=>n.innerText.startsWith('供應商')))"),'independent actor preference');
   await evaluate("window.__trackingQA.change({actorId:'component-a',identity:'session-a'})");await until(()=>evaluate("!Boolean([...document.querySelectorAll('.tracking-sort')].find(n=>n.innerText.startsWith('供應商')))"),'actor preference restored');
   await nodeClick("[...document.querySelectorAll('summary')].find(n=>n.innerText==='欄位設定')");await click('重設欄位配置');assert.ok(await evaluate("Boolean([...document.querySelectorAll('.tracking-sort')].find(n=>n.innerText.startsWith('供應商')))"));
 });
 await check('component-pointer-resize-header-drag-no-accidental-sort',async()=>{
   ...
   const point=await evaluate("(()=>{const r=document.querySelector('[aria-label=調整項目編號欄寬]').getBoundingClientRect();return {x:r.x+r.width/2,y:r.y+r.height/2,w:document.querySelector('th.tracking-reference').getBoundingClientRect().width,sort:document.querySelector('th[aria-sort=descending]')?.innerText||document.querySelector('th[aria-sort=ascending]')?.innerText};})()");
 ...
 await check('component-dirty-navigation-keep-cancel-user-vessel-draft',async()=>{
   await click('＋ 新增／批量新增');await fill('[aria-label="第 1 筆 項目編號"]','LOCAL-DRAFT');
   await click('取消');await click('取消切換');assert.equal(await evaluate("document.querySelector('[aria-label=\"第 1 筆 項目編號\"]').value"),'LOCAL-DRAFT');
   ...
   await evaluate("window.__trackingQA.change({actorId:'component-a',identity:'session-a'})");await until(async()=>(await text()).includes('恢復本船未送出草稿'),'owned draft');await click('恢復本船未送出草稿');assert.equal(await evaluate("document.querySelector('[aria-label=\"第 1 筆 項目編號\"]').value"),'LOCAL-DRAFT');
 });
