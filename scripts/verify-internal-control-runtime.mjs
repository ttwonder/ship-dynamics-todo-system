...
try {
  ...
  assert.ok(internalControlPageSource.includes("useState<InternalControlFilters>(() => emptyFilters(defaultInternalControlVesselIds(user, vessels)))") && internalControlPageSource.includes("const reset = () => {setFilters(emptyFilters(defaultInternalControlVesselIds(user, vessels)));setColumnSort('created-desc');}"), '內控船舶篩選初始值、重設與排序重設都必須使用全部個人經管船舶的default helper');
  assert.ok(internalControlPageSource.includes('重設（我的經管）') && internalControlPageSource.includes('ariaLabel="內控清單船舶篩選"') && !internalControlPageSource.includes('第一艘自管船'), '內控篩選必須由共用船舶選擇器呈現全部／我的經管／自訂多船模式');
