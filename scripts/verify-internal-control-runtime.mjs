...
try {
  ...
  assert.ok(appSource.includes('if(creating&&!requireMutationLease(taskCreationLockKey(candidate.vesselId,candidate.id)))return false') && appSource.includes("if(!creating&&!requireMutationLease(`task:${candidate.id}`))return false") && appSource.includes("if(!requireMutationLease(`task:${candidate.id}`))return false") && appSource.includes("if(!requireMutationLease(`task:${task.id}`))return"), 'task create, task save, per-vessel save, and delete mutation boundaries must require the exact live lease');
