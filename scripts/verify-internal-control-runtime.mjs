...
try {
  ...
  assert.ok(workCenterSource.includes('所選 {printTasks.length+printInternalCases.length} 件'), 'work-center selected-only print total must count selected tasks and standalone internal-control cases');
  assert.ok(workCenterSource.includes('<tbody>{printInternalCases.map(item=>'), 'work-center selected-only print rows must include selected standalone internal-control cases');
  ...
  assert.ok(internalControlPageSource.includes("useState<InternalControlFilters>(() => emptyFilters(defaultInternalControlVesselSelection(user, vessels)))") && internalControlPageSource.includes('const resetSelection=defaultInternalControlVesselSelection(user,vessels);') && internalControlPageSource.includes("const reset = () => {setFilters(emptyFilters(resetSelection));setColumnSort('created-desc');}"), '內控船舶篩選初始值、重設與排序重設都必須使用能保留全部／我的經管模式的default helper');
  assert.ok(internalControlPageSource.includes("重設（{resetSelection.mode==='mine'?'我的經管':'所有船舶'}）") && internalControlPageSource.includes('<VesselListFilter vessels={vessels} mode={filters.ownerMode} selectedVesselIds={filters.vesselIds}') && internalControlPageSource.includes('ariaLabel="內控清單船舶篩選"') && !internalControlPageSource.includes('第一艘自管船'), '內控篩選必須由共用船舶選擇器呈現全部／我的經管／自訂多船模式，且重設文案必須反映真實預設');
  ...
  assert.ok(appSource.includes('const roleVisibleTasks = useMemo(()=>selectOperationalTasks(selectTasksVisibleToUser(data.tasks,currentUser,taskVisibilityRelationships),data.vessels)'), 'App must derive a centralized role-visible task collection');
  ...
  assert.match(appSource, /tasks=\{roleVisibleTasks\}\s+calendarTasks=\{data\.tasks\}\s+internalControlCases=\{roleVisibleData\.internalControlCases\}/, 'vessel dashboard metrics, attention, and summaries must consume only role-visible data while the separately named calendar projection receives complete tasks');
  const totalListStart=appSource.indexOf("tab==='total' && <ListPanel");
  const totalListTag=totalListStart>=0?appSource.slice(totalListStart).split('/>')[0]:'';
  assert.ok(totalListTag.includes('tasks={filteredTasks}') && totalListTag.includes('data={roleVisibleData}'), 'vessel total-list props must not retain hidden internal-control records in the backing data object');
  assert.ok(appSource.includes('const readOnlyTask=taskEditorAuthorizationEpoch===authorizationEpoch?taskReadOnlyData?.tasks.find') && appSource.includes('const editingTask=taskEditorAuthorizationEpoch===authorizationEpoch?((memberEditor.current?.taskId===editingTaskId?memberEditor.current.task:undefined)||readOnlyTask||(creatingTask&&canCreateTasks?selectTasksVisibleToUser([creatingTask],currentUser,taskVisibilityRelationships)[0]:roleVisibleTasks.find(task=>task.id===editingTaskId)||retainedRelatedTask)):undefined;') && appSource.includes('setTaskReadOnlyData(null);'), 'identity, permission, and vessel-scope changes must synchronously suppress stale writable and read-only task editors and drafts');
  ...
  assert.ok(appSource.includes("if(lock.status==='blocked')") && appSource.includes('const leaseRecord=leaseCloudConfigs.current.get(lock.leaseOwnerId)') && appSource.includes("runCloudSaveQueueRpc('釋放多人協作鎖',signal=>releaseEditLock(leaseRecord.sectionKey,lock.leaseOwnerId,leaseRecord.config,signal)") && appSource.includes('leaseCloudConfigs.current'), 'owned and possibly-owned error leases must retain their opaque owner and immutable cloud config until serialized release succeeds');
  ...
  const leaveIdentityStart=appSource.indexOf('const leaveCurrentIdentity = async () => {');
  const leaveIdentityEnd=appSource.indexOf('\n  const readOnlyTask=',leaveIdentityStart);
  assert.ok(leaveIdentityStart>=0&&leaveIdentityEnd>leaveIdentityStart, 'leaveIdentity source boundaries must exist in order');
  const leaveIdentityBranch=appSource.slice(leaveIdentityStart,leaveIdentityEnd);
  assert.ok(leaveIdentityStart>=0&&leaveIdentityBranch.indexOf('await ensureCloudDurableBeforeLeaseRelease')<leaveIdentityBranch.indexOf('await releaseCurrentEditLock()')&&leaveIdentityBranch.indexOf('await releaseCurrentEditLock()')<leaveIdentityBranch.indexOf("setEditingTaskId('');")&&leaveIdentityBranch.indexOf("setEditingTaskId('');")<leaveIdentityBranch.indexOf("setCurrentUserId('')")&&appSource.includes('onClick={() => void leaveCurrentIdentity()}'), 'identity switching must retain sensitive editors and drafts until durable save and serialized lock release succeed, then clear them before changing identity');
  assert.ok(appSource.includes("canEditBusinessContent&&(vesselEditorLeaseAuthorized||Boolean(vesselLeaseIncidentForEditor))&&editingVesselId") && appSource.includes('vesselLeaseIncident.ownerUserId===currentUser.id&&vesselLeaseIncident.authorizationEpoch===authorizationEpoch') && editModalsSource.includes("disabled={saving||leaseMode!=='editable'}") && editModalsSource.includes("<fieldset disabled={leaseMode==='frozen'}") && appSource.includes('editingTask&&taskEditorLeaseAuthorized && <TaskEditModal') && appSource.includes('onClick={resolveEditLockNotice}'), 'identity, permission, and vessel-scope loss must suppress editors; a same-owner same-authorization vessel lease incident may retain only an explicitly non-writable draft while error dismissal still routes through release');
  assert.ok(appSource.includes("currentUser.role!=='vessel'&&canExportReports&&reportPreviewOpen && <ReportPreviewModal data={reportPreviewData}") && appSource.includes('reportPreviewSnapshot.vessels.filter(vessel=>reportPreviewAuthorizedVesselIds.has(vessel.id))') && appSource.includes('const reportPreviewTasks=reportPreviewSnapshot?selectTasksVisibleToUser(reportPreviewSnapshot.tasks,currentUser,{') && appSource.includes('meetings:reportPreviewMeetings') && appSource.includes('tasks:reportPreviewTasks'), 'permission changes must synchronously suppress stale report previews and reauthorize frozen snapshot vessels, tasks, and referenced meetings before rendering');
  ...
  assert.ok(appSource.includes('<TemporaryMeetingsPage loadMeetings={loadMeetingScope} authorizationEpoch={authorizationEpoch} data={roleVisibleData}'), 'meeting consumers must retain centralized role-visible data with only the exact internal loader/epoch props');
  ...
  assert.ok(appSource.includes('if(creating&&!pendingRun&&!requireMutationLease(taskCreationLockKey(candidate.vesselId,candidate.id)))return false') && appSource.includes('if(creating&&pendingRun&&!pendingRun.isCurrent())return false') && appSource.includes('creationSectionKey=taskCreationLockKey(intent.primaryVesselId,intent.taskId)') && appSource.includes("if(!creating&&!requireMutationLease(`task:${candidate.id}`))return false") && appSource.includes("if(!requireMutationLease(`task:${candidate.id}`))return false") && appSource.includes("if(!requireMutationLease(`task:${task.id}`))return"), 'ordinary task create/save and pending-create retry boundaries must require their exact live lease or exact current creation sentinel');
  assert.ok(appSource.includes('onSave={saveVesselEditorDraft}') && appSource.includes('!requireMutationLease(sectionKey)') && appSource.includes('itemLeaseIsAuthorizedInSnapshot(sectionKey,prev)'), 'saved vessel modal drafts must pass through the same live lease and latest-snapshot authorization boundary');
  ...
  assert.ok(appSource.includes('editingTask&&!creatingVisibleTask&&(memberEditor.current?.taskId===editingTask.id||taskEditorReadOnly||mutationLeaseIsOwned(`task:${editingTask.id}`))') && appSource.includes('const taskEditorReadOnly=Boolean((memberEditor.current?.taskId===editingTaskId&&!memberEditor.current.writable)') && appSource.includes('const editingTaskCanMutate=Boolean(editingTask&&taskLockIsAuthorized(editingTask));') && appSource.includes('readOnly={taskEditorReadOnly}') && appSource.includes("if(claimResult==='blocked'){") && appSource.includes("return openTaskReadOnly(task.id,'其他使用者正在編輯此事項'"), 'cloud read-only viewers and lock-blocked users must retain projected task detail access without receiving writable controls or a lease');
  ...
  assert.ok(invalidateStart>=0&&invalidateEnd>invalidateStart, 'invalidate source boundaries must exist in order');
  ...
  assert.ok(appSource.includes("if(lock&&lock.status==='blocked'&&(lock.sectionKey.startsWith('task:')||isTaskCreationLockKey(lock.sectionKey)))") && appSource.includes('雲端設定已變更：已關閉舊工作區的只讀事項'), 'cloud configuration transitions must close an already-open blocked read-only task without consuming a frozen vessel draft');
  ...
  assert.ok(renewalBlockedStart>=0&&renewalBlockedEnd>renewalBlockedStart, 'renewalBlocked source boundaries must exist in order');
  ...
  assert.ok(renewalBlockedBranch.includes("result==='failed'&&taskOpenRequests.current.isCurrent(requestGeneration)") && renewalBlockedBranch.includes('closeTaskEditor(requestGeneration);'), 'a failed renewal-to-read-only transition may consume and restore only its exact still-current return destination');
  ...
  assert.ok(appSource.includes('navigateToTab(k);') && appSource.includes('onBack={closeVesselDetail}') && appSource.includes('onOpenVessel={openVesselDetail}') && appSource.includes("onOpenInternalControl={()=>{if(!canAccessTab(currentUser,'internalControl'))return;navigateToTab('internalControl');}}"), 'tab and vessel-detail navigation handlers must synchronously invalidate pending task opens while forbidden internal-control navigation fails closed');
  assert.ok(appSource.includes('openTask(task,vesselId,vesselId)') && appSource.includes('consumeCurrentTaskEditorSession(taskOpenRequests.current,requestGeneration)') && appSource.includes('returnDestination?.vesselId') && appSource.includes('openVesselEditor(returnDestination.vesselId)'), 'existing tasks opened from a vessel editor must consume only their exact current destination and return to that vessel editor');
  ...
  assert.ok(deleteTaskStart>=0&&deleteTaskEnd>deleteTaskStart, 'deleteTask source boundaries must exist in order');
  assert.ok(appSource.slice(deleteTaskStart,deleteTaskEnd).includes('await runDurableRelatedMutation(`task:${task.id}`') && editModalsSource.includes('if(await onDelete())close()'), 'successful deletion must await its durable related mutation before the modal consumes and restores task return context through the shared close path');
  ...
  assert.ok(appSource.includes('export const cloudIdentity=cloudWorkspaceIdentity') && appSource.includes('cloudConfigIdentity(getSupabaseConfig())') && appSource.includes('sameCloudConfig(currentConfig,record.config)'), 'workspace data identity must exclude rotated anon credentials while operation and mutation boundaries still require exact key, URL, workspace, and table equality');
  ...
  assert.equal(editLockCoordinator.classifyExpiredLeaseRelease('internal-control:case-1',true),'release','the vessel durability exception must not defer stale internal-control or other non-vessel lease cleanup');
  assert.ok(appSource.includes('const renewalCleanupDisposition=classifyExpiredLeaseRelease(lock.sectionKey,vesselSaveLeaseOwners.current.has(lock.leaseOwnerId))') && appSource.includes("renewalCleanupDisposition==='defer-for-durability'") && appSource.includes("runCloudSaveQueueRpc('清理失效單項協作鎖',signal=>releaseEditLock(leaseRecord.sectionKey,lock.leaseOwnerId,leaseRecord.config,signal),8_000)") && appSource.includes('else leaseCloudConfigs.current.delete(lock.leaseOwnerId)'), 'stale definitive blocked renewals must discard non-owner records while stale successes retain hard-timeout release except for an accepted vessel save awaiting durability');
  ...
  assert.ok(appSource.includes("(tab==='internalControl'||tab==='work') && canAccessTab(currentUser,'internalControl') && <InternalControlPage"), 'internal-control page must enforce authorization synchronously at render time during role transitions');
  ...
  assert.ok(removeInternalCaseStart>=0&&removeInternalCaseEnd>removeInternalCaseStart, 'removeInternalCase source boundaries must exist in order');
