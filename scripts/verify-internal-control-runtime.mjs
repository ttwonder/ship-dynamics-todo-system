...
try {
  ...
  assert.ok(appSource.includes("if(renewed.ok){await runCloudSaveQueueRpc('清理失效單項協作鎖',signal=>releaseEditLock(leaseRecord.sectionKey,lock.leaseOwnerId,leaseRecord.config,signal),8_000);leaseCloudConfigs.current.delete(lock.leaseOwnerId);}") && appSource.includes('else leaseCloudConfigs.current.delete(lock.leaseOwnerId)'), 'stale definitive blocked renewals must discard non-owner records while stale successes compensate with a hard-timeout release');
