...
try {
  ...
  assert.ok(workCenterSource.includes('const selectableInternalCases=filteredInternalCases;') && workCenterSource.includes('runSelected(selectedTasks,true,onDismiss)') && workCenterSource.includes('submit(tasks.map(t=>t.id),selectedInternalCases.map(c=>c.id))'), '我的待辦 must let ordinary operators select visible internal-control rows for personal dismissal, completion, or selected PDF without granting delete permission');
  assert.ok(workCenterSource.includes('aria-label={`選取內控') && workCenterSource.includes('runSelected(completableSelectedTasks,canComplete,onBatchComplete)'), '我的待辦 batch completion must submit only completable task IDs while preserving the exact internal-case ID set');
  assert.ok(workCenterSource.includes('runSelected(selectedTasks,canDelete,onBatchDelete)'), '我的待辦 mixed deletion must preserve typed task/internal-case ID sets');
  ...
  assert.ok(internalControlPageSource.includes('aria-label={`選取內控案件') && internalControlPageSource.includes('cases:[...selectedCases]') && internalControlPageSource.includes('const ids=attempt.cases.map(item=>item.id).sort();') && internalControlPageSource.includes("claimItemLease(keys[0],'內控批量結案',keys)") && internalControlPageSource.includes('const closingIds=closureSelection.ids;') && internalControlPageSource.includes('onBatchClose(closingIds,closedDate)'), 'every selectable internal-control row reaches the explicit batch-close handler with the frozen IDs, full admission and chosen date');
  ...
  assert.ok(appSource.includes('const batchCompleteTasks = async (taskIds: string[], internalControlCaseIds: string[] = [], requestedClosedDate?: string) =>')
    && appSource.includes('closeInternalControlCaseBatchFromDraft(draft,liveSelectedInternalCases,liveUser,at,closedDate)')
