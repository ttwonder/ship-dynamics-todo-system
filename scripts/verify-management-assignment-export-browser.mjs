...
import { assignmentExportFixture } from '/scripts/management-assignment-export-fixture.mjs';
import '/src/styles.css';
const data = assignmentExportFixture(createInitialData());
for(let n=1;n<=50;n++) data.vessels.push({...structuredClone(data.vessels[0]), id:'extra-'+n, name:'測試船'+String(n).padStart(2,'0'), shortName:'QA '+n, fullName:'QA EXTRA '+String(n).padStart(2,'0'), assignedUserIds:['a'], delegateManagers:[]});
...
try {
  ...
  assert.match(await evaluate("document.querySelector('.management-assignment-paper').innerText"), /測試督導甲\n（代理：測試代理丙）/);
