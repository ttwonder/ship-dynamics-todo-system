   ...
   try{
    ...
    if(url.pathname.startsWith('/rest/v1/')){
     ...
     try{
      ...
      metrics.push({rpc:name,...(shipTracking&&name==='ship_dynamics_tracking_public_v1'?{action:body.p_action}:{}),status:value?.ok===false?value.code:'SQL_OK',conflictKey:value?.conflict_key,operationId:body.p_operation_id,vesselId:body.p_vessel_id,revision:value?.revision,bytes:Buffer.byteLength(JSON.stringify(value??null)),elapsedMs:performance.now()-start,...(trace?{trace}:{})});
