...
const lifecycleSource=source('src/vesselLifecycle.ts'),lifecycleFile=parse(lifecycleSource);
const operationalCasesDeclaration=lifecycleFile.statements.find(n=>ts.isFunctionDeclaration(n)&&n.name?.text==='selectOperationalCases');
assert.ok(operationalCasesDeclaration);
const selectOperationalCases=evaluate(operationalCasesDeclaration.getText(lifecycleFile).replace(/^export\s+/,''),{});
const observations=[];
