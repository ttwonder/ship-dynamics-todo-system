...
try{
 ...
 const ledger=async(names=tables)=>{const out={};for(const name of names){assert.match(name,/^[a-z0-9_]+$/);out[name]=await q(observer,`select coalesce(jsonb_agg(to_jsonb(t) order by to_jsonb(t)::text),'[]') r from ${name} t`);}return out;};
 ...
 await check('NI02-atomic-installer-preserves-all-58-legacy-tables',async()=>{await observer.query(install);assert.deepEqual(await ledger(),before);assert.equal(await q(observer,'select count(*)::int r from ship_dynamics_record_workspaces'),0);});
 await check('NI03-independent-readback-and-rerun-refusal',async()=>{const rs=await a.query(fs.readFileSync('supabase/release/06_verify_record_storage.sql','utf8'));const out=rs.flatMap(r=>r.rows??[]).find(r=>r.install_readback)?.install_readback;assert.ok(out);assert.equal(out.api_count,52);assert.deepEqual(out.api_missing_or_invalid,[]);assert.equal(out.record_tables_private,true);receipt.installReadback=out;await assert.rejects(()=>observer.query(install),e=>e.message==='release-records-already-present-use-readback');await observer.query('rollback');assert.deepEqual(await ledger(),before);});
