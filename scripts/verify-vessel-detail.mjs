...
const tasks = [
  ...
  task('aggregate','other','中',false,'2026-07-22','2026-07-14','多船專題',{vesselIds:['other','v1']}),
  task('foreign','v2','急',false,'2026-07-19','2026-07-15','其他船舶'),
];

assert.deepEqual(selectVesselDetailTasks(tasks,'v1',{closedMode:'all',priority:'all',query:'',sort:'priority'}).map(item=>item.id),['urgent-open','high-closed','aggregate','low-open'],'应依关注程度排序且纳入多船聚合事项');
assert.deepEqual(selectVesselDetailTasks(tasks,'v1',{closedMode:'open',priority:'all',query:'',sort:'due-asc'}).map(item=>item.id),['urgent-open','aggregate','low-open'],'未结筛选应依期限近到远');
...
for (const label of ['回到船隊看板','船舶基本資料','航行與港口','時間與資料來源','貨載資訊','動態與備註','待辦事項清單']) assert.ok(detail.includes(label), `详情页缺少 ${label}`);
