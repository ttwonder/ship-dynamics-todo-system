...
try{
 ...
 for(const sourceAuthority of [null,{}, {...binding,workspace:'other'},{...binding,source:'unknown'},{...binding,epoch:2},{...binding,admitted:false}]){await assert.rejects(()=>m.saveManualItineraryDailyReport({...old,sourceAuthority},config,client));count++;}assert.equal(calls.length,0);
