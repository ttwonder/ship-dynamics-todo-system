...
try {
  ...
  const bootstrapRemoteThen=appSource.indexOf('.then(({remote,binding,base,floor,readScope}) =>',bootstrapStart);
  ...
  assert.ok(compactAppSource.includes('consthistoryIdentity=authorityFloorIdentity(identity,binding)')&&compactAppSource.includes('constbase=parseConfirmedCloudBase(localStorage.getItem(CLOUD_CONFIRMED_BASE_KEY),historyIdentity)')&&compactAppSource.includes('trustedPersistedBaseForRemote(persistedConfirmedBase,remote,appDataContentEqual)'), 'bootstrap必須只從同workspace與已確認source/epoch的持久化confirmed envelope恢復可信共同base；實際分支另由bootstrap effect regression驗證');
  ...
  assert.ok(compactAppSource.includes('constfloor=Math.max(durableCloudRevisionFloors.current.get(historyIdentity)??-1,base?.revision??-1)')&&compactAppSource.includes('persistedDurableFloor=Math.max(floor,durableCloudRevisionFloors.current.get(boundHistoryIdentity)??-1)')&&compactAppSource.includes('durableCloudRevisionFloors.current.set(boundHistoryIdentity,persistedDurableFloor)')&&compactAppSource.includes('constpersistedRemoteRollback=remote.revision<persistedDurableFloor')&&compactAppSource.includes('localContentDiverged||persistedRemoteRollback'), 'bootstrap必須拒絕低於workspace/source持久化durable floor的remote rollback，即使單一confirmed envelope屬於其他來源');
  assert.ok(compactAppSource.includes('parseDurableRevisionFloors(localStorage.getItem(CLOUD_REVISION_FLOORS_KEY))')&&compactAppSource.includes('durableRevisionFloorRegistryValid')&&compactAppSource.includes('updateDurableRevisionFloor(durableCloudRevisionFloors.current,identity,snapshot.revision)')&&compactAppSource.includes('localStorage.setItem(CLOUD_REVISION_FLOORS_KEY,serializeDurableRevisionFloors(')&&compactSyncLatestSource.includes('durableCloudRevisionFloors.current.get(authorityFloorIdentity(syncIdentity,originalAuthority.current))??-1')&&compactSyncLatestSource.includes('remote.revision<durableRevisionFloor'), 'durable revision floor registry must survive A -> B -> reload -> A, remain monotonic per workspace/source, and distinguish corruption from first use');
  assert.ok(compactAppSource.includes('assertRemoteExtendsDurableHistory(activeCloudIdentity.current,base,remote,pending.authority)')&&compactAppSource.includes('identity=authorityFloorIdentity(identity,binding);')&&compactAppSource.includes('assertRemoteExtendsDurableHistory(cloudIdentity(token.config),confirmedCloudData.current,remote)'), 'autosave conflict must use its immutable command-authority history floor; batch discard must still reject rollback or same-revision rewritten remotes before adoption');
