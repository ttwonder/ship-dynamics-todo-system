...
const checks = [
  ...
  ['每日早會報告需固定使用全部啟用船舶', app.replace(/\s/g,'').includes('constreportVessels=data.vessels.filter(v=>v.isActive)') && !app.includes("請至少選擇一艘船舶再預覽報告")],
  ...
  ['會議跟進事項需避免重複', meetingTasks.includes('canonicalByKey') && meetingTasks.includes("task.sourceMeetingId === meetingId") && meetingTasks.includes('重複的臨會/專題待辦')],
