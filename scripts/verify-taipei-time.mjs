...
assert.match(dashboardSource,/taipeiDateKey\(vessel\.updatedAt \|\| vessel\.position\.updatedAt\) === todayDate\(\)/,'dashboard updated-today metrics must classify instants by Taipei date');
assert.doesNotMatch(dashboardSource,/\(vessel\.updatedAt \|\| vessel\.position\.updatedAt\)\.slice\(0, 10\)/,'dashboard must not compare a UTC date slice with a Taipei business date');
