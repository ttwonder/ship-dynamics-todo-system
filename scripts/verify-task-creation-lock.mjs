...
try {
  ...
  assert.ok(creationFlowSource.includes('taskCreationRelatedLockKeys(taskVesselIds(candidate),candidate,isMeetingTaskSource(candidate))')&&creationFlowSource.includes('acquireEditLockBundle(')&&creationFlowSource.includes('transientCloudBlockLockGuards.current.set'), 'creation save must derive, acquire, and submit every existing vessel/relation lock touched by the task projection');
  ...
  assert.ok(app.includes('const pending=pendingEntry.value;')&&app.includes('const {snapshot:next,baseSnapshot:base,token,savedBy,actorUserId,identityGeneration,lockGuards,isCurrent,canSubmit}=pending')&&app.includes('identitySessionGeneration.current===identityGeneration')&&app.includes('buildCloudBlockPatch(remote,candidate,storageRemote)'), 'atomic block retry must retain and replay the exact per-intent unrendered submitted creation snapshot and durable base');
  ...
  assert.ok(app.includes('const capturedCreationConfig=pendingRun?.config||capturedCreationLease!.config')&&app.includes('recoveredRemote=await fetchCloudData(capturedCreationConfig)')&&app.includes('if(creationIsCurrent()){')&&app.includes('confirmCloudSnapshot(cloudIdentity(capturedCreationConfig),recoveredRemote)'), 'creation outcome classification must use the immutable config captured by the editor lease or pending-intent run even after session/config staleness, while publishing recovery only for the current session');
  ...
  assert.ok(batchEditor.includes('if(await save(structuredClone(draftVessels)))afterSave?.();')&&batchEditor.includes('void submit(()=>onAddTask(vessel.id))')&&app.includes('return closeBatchManaged(mutationAuthorization);')&&app.includes('onAddTask={id=>{void addTaskForVessel(id,false,true,renderedBatchTaskReturnContext);}}'), 'batch-managed add-task must save and release the exact captured batch session bundle before using the common creation-lock entry');
