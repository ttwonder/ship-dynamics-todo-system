...
export async function exportChecks(c){
 ...
 await check('download-all-filtered-sorted-not-30-and-visible-column-order',async()=>{
  await nodeClick("document.querySelector('.tracking-preferences summary')");await nodeClick("[...document.querySelectorAll('.tracking-preferences label')].find(n=>n.innerText==='原項次').querySelector('input')");
