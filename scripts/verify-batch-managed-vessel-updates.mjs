...
const discardPostReleaseConfigGuard=discardBranch.indexOf('if(!discardConfigIsCurrent())throw new StaleAsyncConfigError();',discardPostReleaseGuard);
assert.ok(discardStart>=0&&discardBranch.includes('const operation=beginBatchManagedOperation()')&&discardBranch.indexOf('fetchCloudData')<discardRelease&&discardRelease<discardPostReleaseGuard&&discardPostReleaseGuard<discardPostReleaseConfigGuard&&discardPostReleaseConfigGuard<discardBranch.indexOf("detachBatchManagedState('')")&&discardBranch.indexOf("detachBatchManagedState('')")<discardBranch.indexOf('setData(remote)'), '放棄流程必須捕獲own session/config/locks，先取得可信remote，再釋鎖並重驗完整config，之後才可關閉自己的modal並替換本機');
