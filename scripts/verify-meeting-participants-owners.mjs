...
assert.match(page, /請填寫召開緣由/, '保存时需拒绝空召開緣由');
assert.doesNotMatch(page, /請填寫會議決議/, '會議決議不應再是必填');
assert.match(page, /請選擇召開日期/, '保存时需拒绝空召开日期');
assert.doesNotMatch(page, /請選擇預計完成日期/, '保存時不得拒絕空白預計完成日期');
assert.match(page, /<label>預計完成日期<\/label><input type="date"/, '預計完成日期需保留為選填欄位');
assert.match(page, /ariaLabel="召開緣由" required/, '召開緣由欄位需標示必填');
assert.doesNotMatch(page, /ariaLabel="決議／會議結論" required/, '決議／會議結論欄位不得標示必填');
...
assert.match(page, /const canDeleteMeetings = \(currentUser\.role === 'owner' \|\| currentUser\.role === 'admin'\) && editable/, '臨會刪除鍵只能給 Owner／管理員且需具備臨會編輯權限');
assert.match(page, /只有 Owner／管理員可以刪除臨會\/專題/, '臨會刪除需有權限拒絕訊息');
assert.match(page, /sourceMeetingId !== meeting\.id/, '刪除臨會時需同步刪除該臨會產生的待辦');
assert.match(page, /刪除會議/, '臨會詳情需提供刪除會議按鈕');
assert.match(page, /deleteMeeting\(meeting\)/, '臨會總清單每列需提供刪除按鈕');
assert.match(workflow, /ownerUserIds\?: string\[\]/, '会议待办 reconcile 需接收负责人');
assert.match(page,/sanitizeMeetingStatusMutation\(/,'會議保存端需重新驗證歷程新增、刪除與作者來源');
assert.match(page,/isAbnormal:requestedDraft\.isAbnormal\|\|requestedDraft\.isInternalControl/,'父會議內部管控必須同步 canonical 異常旗標');
assert.match(page,/taskIsClosedForScope\(task,taskVesselIds\(task\)\)/,'關聯分船待辦顯示需使用 canonical 範圍結案語意');
assert.doesNotMatch(page,/task\.isClosed \? '已結案'/,'關聯待辦不得用 raw isClosed 顯示完成狀態');
assert.match(page,/completedBy:previousCompleted\?\(liveMeeting\?\.completedBy\|\|liveUser\.id\):liveUser\.id/,'會議完成人必須依 live 狀態轉換及目前操作人蓋章');
...
try {
  ...
  const { sanitizeMeetingStatusMutation } = await server.ssrLoadModule('/src/meetingStatusWorkflow.ts');
  const base = {
    revision: 1,
    ...
    meetings: [
      { id: 'm-old-wait', subject: '旧待开', status: '待開會', meetingDate: '2026-07-19', vessels: [], departments: ['航務部'], participantUserIds: ['u1', null], responsibleUserIds: ['u2', { bad: true }], resolution: '决议', expectedDate: '2026-07-20', priority: '中', taskItems: [{id:'blank',description:'   '},{id:'trimmed',description:'  有效事項  '}], createdBy: 'owner', createdAt: '2026-07-19' },
  ...
  assert.deepEqual(normalized.meetings[0].taskItems.map(item=>({id:item.id,description:item.description})),[{id:'trimmed',description:'有效事項'}],'正規化必須移除空白會議事項並修剪有效描述');
  ...
  const previousLogs=[
    {id:'other-log',at:'2026-07-18T00:00:00.000Z',by:'工務乙',byUserId:'u2',text:'他人歷程'},
    {id:'own-log',at:'2026-07-17T00:00:00.000Z',by:'航務甲',byUserId:'u1',text:'本人歷程'},
  ];
  const actor={id:'u1',name:'航務甲',role:'operator'};
  assert.equal(sanitizeMeetingStatusMutation([previousLogs[1]],previousLogs,actor,'2026-07-22T00:00:00.000Z',()=> 'new-id').ok,false,'非管理員不得刪除他人會議歷程');
  assert.equal(sanitizeMeetingStatusMutation([{...previousLogs[0],text:'偽造改寫'},previousLogs[1]],previousLogs,actor,'2026-07-22T00:00:00.000Z',()=> 'new-id').ok,false,'既有會議歷程不得改寫');
  const trustedMeetingLogs=sanitizeMeetingStatusMutation([
    {id:'forged',at:'2000-01-01T00:00:00.000Z',by:'Owner',byUserId:'owner',text:'新增進度'},
    previousLogs[0],
  ],previousLogs,actor,'2026-07-22T00:00:00.000Z',()=> 'server-log');
  assert.equal(trustedMeetingLogs.ok,true,'本人可刪除自己的舊歷程並新增狀態');
  assert.deepEqual(trustedMeetingLogs.logs?.[0],{id:'server-log',at:'2026-07-22T00:00:00.000Z',by:'航務甲',byUserId:'u1',text:'新增進度'},'新增會議歷程的 ID、時間與作者必須由保存端蓋章');
  assert.deepEqual(trustedMeetingLogs.logs?.[1],previousLogs[0],'未刪除的既有會議歷程必須逐字保留');
  assert.equal(trustedMeetingLogs.latestStatus,'新增進度','最新狀態必須由可信歷程首筆推導');
  assert.equal(sanitizeMeetingStatusMutation([
    {id:'forged',at:'2000-01-01T00:00:00.000Z',by:'Owner',text:'新狀態'},
    ...previousLogs,
  ],previousLogs,actor,'2026-07-22T00:00:00.000Z',()=> 'other-log').ok,false,'新增會議歷程 ID 不得碰撞既有歷程');
  let generatedIndex=0;
  assert.equal(sanitizeMeetingStatusMutation([
    {id:'new-1',at:'',by:'',text:'新增一'},
    {id:'new-2',at:'',by:'',text:'新增二'},
    ...previousLogs,
  ],previousLogs,actor,'2026-07-22T00:00:00.000Z',()=> ['dup','dup'][generatedIndex++]||'dup').ok,false,'同批新增會議歷程 ID 碰撞必須原子拒絕');
