...
for(const [id,set] of [
 ['C15-management-private',e=>e.hasManagementPrivateDraft=()=>true],
...
const base=execFileSync('git',['show','53e230329f250045f0c971154729b70a223e7fba:src/App.tsx'],{encoding:'utf8'});
await test('C16-private-feedback-projection-no-transient',async()=>{
 const code=ts.transpileModule(['visibleSavePhase','visibleSaveToast','visibleSaveStatus'].map(declaration).join('\n')+';return {phase:visibleSavePhase,toast:visibleSaveToast,status:visibleSaveStatus};',{compilerOptions:{target:ts.ScriptTarget.ES2022}}).outputText;
 // Only trusted local App AST is executed; no user/network text enters Function.
 const project=new Function('env','with(env){'+code+'}');
 for(const dirty of [true,false])for(const phase of ['saved','saving','dirty','error'])for(const kind of ['success','warning','error']){
  const toast={kind},status='confirmed model status',v=project({hasManagementPrivateDraft:()=>dirty,savePhase:phase,saveToast:toast,visibleCloudStatus:status});
  assert.equal(v.phase,dirty&&phase==='saved'?'dirty':phase);assert.equal(v.toast,dirty&&kind==='success'?null:toast);assert.equal(v.status,dirty&&phase==='saved'?'修改仍保留在目前頁面，請不要關閉。':status);
 }
});
...
const privateProps='key={`${currentUser.id}:${identitySessionGeneration.current}`} onPrivateDraftChange={onManagementPrivateDraftChange} ';
assert.equal(source.split(privateProps).length,2,'one exact approved Management private registration');
const oldGuidance='<span>請先點擊上方的保存按鈕，並等待雲端確認。</span>';
const privateGuidance="<span>{hasManagementPrivateDraft()?'管理表單尚有未提交的修改，請回到各表單按保存；上方保存不會提交這些欄位。':'請先點擊上方的保存按鈕，並等待雲端確認。'}</span>";
assert.equal(base.split(oldGuidance).length,2,'one original guidance span');
assert.equal(source.split(privateGuidance).length,2,'one exact approved private form guidance span');
const old=boundary(base.replace(oldGuidance,privateGuidance)),now=boundary(source.replace(privateProps,'').replaceAll('visibleSavePhase','savePhase').replaceAll('visibleSaveToast','saveToast').replaceAll('visibleSaveStatus','visibleCloudStatus'));assert.deepEqual(now.jsx,old.jsx,'all JSX roots unchanged except exact private registration and form-save guidance');assert.deepEqual([...now.literals].filter(v=>!old.literals.has(v)),['./managementDraft'],'only approved non-visible module import');
