...
try {
  ...
  const summaries = await daily.listItineraryDailyReports('owner-1', config, client);
  assert.equal(summaries.length, 1);
  assert.equal(summaries[0].businessDate, '2026-09-04');
  assert.equal(summaries[0].vesselCount, 39);
  assert.equal(Object.hasOwn(summaries[0], 'snapshot'), false);
  assert.deepEqual(calls[0], {
    name: 'sd_itinerary_daily_report_list',
    params: { p_workspace_key: 'workspace-a', p_actor_user_id: 'owner-1' },
  });
  ...
  assert.deepEqual(calls[2], {
    ...
    params: {
      ...
      p_operation_id: request.operationId, p_expected_dates: ['2026-09-04'], p_delete_dates: ['2026-09-04'],
    },
  });
  ...
  for (const tampered of [
    ...
    { ...pending, deleteDates:['2026-09-03'] },
  ]) {
  ...
  await assert.rejects(
    daily.listItineraryDailyReports('owner-1', config, unavailableClient),
