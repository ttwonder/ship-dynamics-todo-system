...
try{
  ...
  assert.ok(workCenter.includes('const selectableTasks=filteredTasks;')
    ...
    && workCenter.includes('const completeSelected=()=>runSelected(completableSelectedTasks,canComplete,onBatchComplete)')
    && workCenter.includes('const committed=await submit(tasks.map(t=>t.id),selectedInternalCases.map(c=>c.id))')
    && workCenter.includes('onClick={completeSelected}')
    && workCenter.includes('await runSelected(selectedTasks,true,onDismiss)')
