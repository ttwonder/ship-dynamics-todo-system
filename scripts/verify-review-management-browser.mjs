...
const receipt={kind:'original-App-native-PG-multi-context',inputHead:execFileSync('git',['rev-parse','HEAD'],{encoding:'utf8'}).trim(),status:'RUNNING',cases:[],network:[],errors:[],blockedExternal:[],commands:[{command:'node scripts/verify-review-management-browser.mjs',exit:null}],productionContacted:false};
...
try{
 ...
 qa=await createRecordStorageLocalQa({browserAuthority:true,internalControl:true,taskMember:true,scopedRead:true,performanceTrace:true,legacySnapshot:process.env.QA_MGACK_MODE==='handover-legacy',handoverMigrationFixture:process.env.QA_HANDOVER_UPGRADE==='1'?async(db,phase)=>(await import('./vessel-manager-handover-native.mjs')).rehearseHandoverMigration({db,phase,receipt}):null,preparePerformanceFixture:async initial=>{
  ...
  if((process.env.QA_MGACK_MODE||'').startsWith('handover')||(process.env.QA_MGACK_MODE||'').startsWith('r3-'))(await import('./vessel-manager-handover-native.mjs')).prepareHandover(initial);
  if(process.env.QA_MGACK_MODE==='r3-f2')initial.vessels[0].delegateManagers=[{userId:'qa-spare',isActive:true}];
 ...
 ws.addEventListener('message',event=>{const m=JSON.parse(event.data);if(m.id){const p=pending.get(m.id);if(p){pending.delete(m.id);m.error?p.reject(new Error(m.error.message)):p.resolve(m.result);}return;}
  const handle=async()=>{
   ...
   if(m.method==='Page.javascriptDialogOpening'){
    ...
    if(!['目前登入的 Owner 不可降級自己','管理員不可建立或修改 Owner 帳號'].includes(message)&&!accept&&!privatePrompt&&type!=='beforeunload'&&message!=='進站密碼已更新'&&!(currentCase==='MG-PRIVATE-GENERIC-ACK'&&message==='個人密碼已更新；下次登入需使用新密碼。'))receipt.errors.push('unexpected dialog: '+message);await call('Page.handleJavaScriptDialog',{accept},m.sessionId);
   }
 ...
 receipt.probeInputs=Object.fromEntries(['verify-review-management-browser.mjs','review-management-cases.mjs'].map(p=>[p,hash(fs.readFileSync('scripts/'+p,'utf8'))]));
 assert.ok(['r3-f1','r3-f2','r3-f3','r3-f6'].includes(mode));
 await(await import('./review-management-cases.mjs')).runR3({a,qa,read,call,until,wait,write,receipt,setCase:value=>currentCase=value,freshReadback,native,makePage,login,mode});receipt.status='PASS';
