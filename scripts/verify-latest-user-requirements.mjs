...
try {
  ...
  assert.ok(app.includes("const needsPassword=user.role==='owner'||user.role==='admin'||Boolean(user.passwordHash)") && app.includes('if(!needsPassword){setCurrentUserId(user.id);return;}') && app.includes('if(await sha256(pw)!==user.passwordHash)'), '登入密碼驗證應套用 Owner／管理員或已設定個人密碼者；無密碼非管理角色仍需在比對密碼前直接登入');
