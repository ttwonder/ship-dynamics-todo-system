...
try {
  ...
  assert.ok(app.includes('assignedOwnerUserIds') && app.includes('liveVessel.assignedUserIds'), '新增要事取得協作鎖後，必須以最新主資料自動帶入管理頁已分配的船舶經管人員');
  assert.ok(!taskEditor.includes('assignedUserIds=') && !taskEditor.includes('managedVesselIds='), '事项涉及人员不得修改管理页船舶／人员分管');
  assert.ok(app.includes('createTaskOpenRequestCoordinator') && app.includes("taskOpenRequests.current.begin({vesselId:returnToVessel?vesselId:'',batchManaged:returnToBatchManaged,...(batchContext?{batchContext}:{})})") && app.includes('consumeCurrentTaskEditorSession(taskOpenRequests.current,requestGeneration)') && app.includes('returnDestination?.batchManaged') && app.includes('openVesselEditor(returnDestination.vesselId)') && app.includes('addTaskForVessel(id,true)'), '從快速更新進入新增要事後，取消或保存必須按精確session token只消耗最新返回目的地並返回快速更新彈窗');
  ...
  assert.ok(
    ...
    '每日早會報告只能使用目前授權船舶；有選擇與歷史快照時都必須再次套用授權交集',
  );
  ...
  assert.ok(!cloudSource.includes('needsPasswordResetPersistence') && !cloudSource.includes('persistPasswordMigrationCas') && app.includes('localContentDiverged=hasLocalCache&&!appDataContentEqual(data,remote)') && app.includes('confirmedCloudData.current=recoveredBase') && app.includes('const persistedRemoteMissing=persistedDurableFloor>=0') && app.includes('confirmedCloudData.current=persistedConfirmedBase') && app.includes('identityChanged || unknownDirtyCache || persistedRemoteMissing'), '读取／normalize 不得自动匿名写回密码迁移；内容分歧不得伪造共同base，remote缺失时须依workspace durable floor保留可信base并阻挡写入');
  ...
  assert.ok(styles.includes('.batch-task-table th:nth-child(7)') && styles.includes('.batch-task-table th:nth-child(8)') && styles.includes('.batch-task-table th:nth-child(11)') && styles.includes('min-width:260px'), '待辦總表部門/追蹤窗口/狀態欄寬需明確調整');
  ...
  assert.ok(app.includes("filter-reset-btn\" onClick={()=>{setFilters({...emptyFilters,closedMode:filters.closedMode});setColumnSort('created-desc');}}>清除篩選</button><button className=\"btn small ghost\" onClick={toggleAll}"), '清除篩選按鈕需移到全選本頁前面，並同步恢復預設排序');
