...
try {
  ...
  assert.ok(app.includes('createTaskOpenRequestCoordinator') && app.includes("taskOpenRequests.current.begin({vesselId:returnToVessel?vesselId:'',batchManaged:returnToBatchManaged})") && app.includes('consumeCurrentTaskEditorSession(taskOpenRequests.current,requestGeneration)') && app.includes('returnDestination?.batchManaged') && app.includes('openVesselEditor(returnDestination.vesselId)') && app.includes('addTaskForVessel(id,true)'), '從快速更新進入新增要事後，取消或保存必須按精確session token只消耗最新返回目的地並返回快速更新彈窗');
