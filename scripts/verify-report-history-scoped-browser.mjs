...
const receipt={kind:'original-App-native-PG-multi-context',inputHead:execFileSync('git',['rev-parse','HEAD'],{encoding:'utf8'}).trim(),status:'RUNNING',cases:[],network:[],errors:[],blockedExternal:[],commands:[{command:'node scripts/verify-report-history-scoped-browser.mjs',exit:null}],productionContacted:false};
receipt.inputs=Object.fromEntries(['scripts/verify-report-history-scoped-browser.mjs','scripts/record-storage-native-qa.mjs','scripts/record-storage-local-qa.mjs','scripts/record-internal-control-local-fixture.mjs','src/main.tsx','src/App.tsx','src/ReportDailyHistories.tsx','src/morningHistory.ts','src/normalize.ts','src/types.ts','src/DataAnalysis.tsx','src/dataAnalysisVesselAttention.ts','src/taskVesselProgress.ts','src/taskVesselScope.ts','src/taskCategories.ts','src/taskAttention.ts','src/vesselAttention.ts','src/meetingVesselAttention.ts','src/taipeiTime.ts','src/cloud.ts','src/cloudRecordScopes.ts','src/cloudBlockPatch.ts','scripts/report-history-save-oracle.mjs','scripts/report-history-model-probes.mjs','supabase/development/20260908_appdata_record_scoped_read.sql'].map(p=>[p,hash(fs.readFileSync(p,'utf8'))]));
...
try{
 ...
 qa=await createRecordStorageLocalQa({internalControl:true,taskMember:true,scopedRead:true,performanceTrace:true,preparePerformanceFixture:async (initial,vite)=>{
  ...
  for(const [at,title] of [['2026-08-03T02:00:00.000Z','RH FROZEN A'],['2026-08-04T02:00:00.000Z','RH FROZEN B']]){
   const result=upsertDailyMorningReport(initial,{at,actorUserId:'qa-owner',source:'manual'});assert.equal(result.status,'saved');
   result.report.title=title;if(title==='RH FROZEN B')result.report.vesselIds=['qa-v1'];result.report.snapshot.vessels[0].note.recentDynamics=title;
   result.report.snapshot.qaUnselected='QA_UNLOADED_DETAIL_SENTINEL'.repeat(12000);
   initial.agendaReports=result.data.agendaReports;
  }
  initial.agendaReports.push({id:'rh-no-snapshot',title:'RH ABSENT',businessDate:'2026-08-05',kind:'daily-morning',vesselIds:['qa-v1'],createdBy:'qa-owner',createdAt:'2026-08-05T02:00:00.000Z',taskCount:0});
  initial.agendaReports.push({...initial.agendaReports.at(-1),id:'rh-malformed',title:'RH MALFORMED',snapshot:{vessels:[],tasks:[]}});
  const related=initial.tasks.find(t=>t.id==='qa-unrelated-task'),meeting=initial.meetings[0];
  related.sourceMeetingId=meeting.id;related.sourceMeetingItemId='rh-decision';related.sourceType='temporary';related.attentionDimension='meeting';related.distributeToVessels=true;related.vesselIds=['qa-v1','qa-v2'];meeting.vessels=['qa-v1','qa-v2'];meeting.includeInMorning=true;
  meeting.taskItems=[{id:'rh-decision',description:related.description,categories:related.categories,distributeToVessels:true}];
  ...
  for(const row of [...initial.tasks,...initial.internalControlCases,meeting])row.statusLogs=Array.from({length:5},(_,i)=>({id:'rh-history-'+row.id+'-'+i,at:'2026-08-06T02:00:00.000Z',by:'QA OWNER',byUserId:'qa-owner',text:i===0?(row.status||'RH COMPLETE HISTORY 0'):'RH COMPLETE HISTORY '+i}));
  for(const item of initial.internalControlCases){const task=initial.tasks.find(t=>t.id===item.linkedTaskId);if(task)item.statusLogs=structuredClone(task.statusLogs);}
  related.vesselProgress=['qa-v1','qa-v2'].map(vesselId=>({vesselId,status:related.status,isClosed:false,updatedAt:'2026-08-06T02:00:00.000Z',updatedBy:'qa-owner',statusLogs:related.statusLogs.map(l=>({...l,id:l.id+'-'+vesselId}))}));
 ...
 const physical=async()=> (await native.observer.query("select collection,entity_id,to_jsonb(t) value,xmin::text,ctid::text from ship_dynamics_records t order by collection,entity_id")).rows;const physicalBefore=await physical();
 const original=await read();fs.writeFileSync(path.join(run,'sql-before.json'),JSON.stringify(scrub(original),null,2));
 const {normalizeAppData}=await qa.loadModule('/src/normalize.ts'),scopes=await qa.loadModule('/src/cloudRecordScopes.ts'),history=await qa.loadModule('/src/morningHistory.ts'),{MorningDailyHistoryPanel}=await qa.loadModule('/src/ReportDailyHistories.tsx');
 ...
 const render=data=>renderToStaticMarkup(React.createElement(MorningDailyHistoryPanel,{reports:history.dailyMorningReports(data.agendaReports),onOpen:()=>{}}));
 const expected=history.dailyMorningReports(model.agendaReports).map(r=>r.title);assert.deepEqual(expected,['RH FROZEN B','RH FROZEN A']);
 fs.writeFileSync(path.join(run,'native-model-parity.json'),JSON.stringify({fullMarkup:render(model),homeMarkup:render(home),fullIds:history.dailyMorningReports(model.agendaReports).map(r=>r.id),homeIds:history.dailyMorningReports(home.agendaReports).map(r=>r.id)},null,2));
 currentCase='RH-LIST';await a.click('報告中心');await until(()=>a.eval("Boolean(document.querySelector('.morning-daily-history-panel'))"),'original report center');await a.sync();
 ...
 assert.equal(render(model),render(home),'original full and home list markup parity');
 receipt.cases.push({caseId:currentCase,layer:'original-UI-native-PG',status:'PASS'});
 receipt.cases.push({caseId:'RH-NATIVE-LIST-PARITY',layer:'native-model-original-component',status:'PASS'});
 const {reportHistoryModelProbes}=await import('./report-history-model-probes.mjs');receipt.cases.push(...await reportHistoryModelProbes(qa,native,run,model,home));
 const preview=()=>a.eval("Boolean(document.querySelector('#report-preview-title'))");
 const openHistory=async(p,title)=>{await until(()=>p.eval("Boolean(document.querySelector('.morning-daily-history-panel .saved-report button'))"),'history page ready');await p.activate(`[...document.querySelectorAll('.morning-daily-history-panel .saved-report')].find(n=>n.querySelector('b').innerText===${JSON.stringify(title)})?.querySelector('button')`);await until(()=>p.eval("Boolean(document.querySelector('#report-preview-title'))"),'exact history preview');assert.ok((await p.eval("document.querySelector('.report-paper').innerText")).includes(title),'actual frozen vessel dynamics');};
 currentCase='RH-OPEN-BACK';
 const locator="document.querySelector('input[aria-label=每日早會歷史日期]')";
 await a.eval(`(${locator}).focus()`);await call('Input.dispatchKeyEvent',{type:'keyDown',key:'ArrowUp',code:'ArrowUp'},a.s);await call('Input.dispatchKeyEvent',{type:'keyUp',key:'ArrowUp',code:'ArrowUp'},a.s);
 const dateValue=await a.eval(`(${locator}).value`);
 await openHistory(a,'RH FROZEN A');await a.screen('history-a');
 assert.ok((await a.text()).includes('2026-08-03'));await a.click('關閉');await until(async()=>!await preview(),'back from A');assert.equal(await a.eval(`(${locator}).value`),dateValue);
 const b=await makePage('qa-operator',(await call('Target.createBrowserContext')).browserContextId);await login(b);await b.click('報告中心');await openHistory(b,'RH FROZEN B');await b.screen('history-b-second-actor');
 await openHistory(a,'RH FROZEN A');assert.ok((await a.text()).includes('RH FROZEN A'));assert.ok((await b.text()).includes('RH FROZEN B'));await a.click('關閉');await b.click('關閉');
 const targetReads=receipt.network.filter(r=>r.caseId===currentCase&&r.readScope==='targets');await until(()=>targetReads.every(r=>r.finished),'target bodies complete');
 assert.ok(targetReads.some(r=>JSON.stringify(r.targets)===JSON.stringify([{collection:'agendaReports',id:'daily-morning-2026-08-03'}])));
 for(const r of targetReads){const body=JSON.parse(fs.readFileSync(path.join(run,'response-'+receipt.network.indexOf(r)+'.json')));for(const row of body.collections.agendaReports.rows)if(row.value.snapshot)assert.ok(r.targets.some(t=>t.id===row.id),'no unselected snapshot body');}
 ...
 assert.deepEqual(await read(),original,'all history reads unchanged SQL');
 currentCase='RH-SAVE';await a.click('報告中心');await a.sync();saveBefore=await read();saveStarted=Date.now();
 fs.writeFileSync(path.join(run,'save-before-sql.json'),JSON.stringify(scrub(saveBefore),null,2));
 qa.setRecordFault({before:async({name,body})=>{if(name!==patchRpc)return;assert.equal(expectedSave,null,'one original save');const {reportSaveExpected}=await import('./report-history-save-oracle.mjs');expectedSave=await reportSaveExpected(saveBefore,body,qa,saveStarted);fs.writeFileSync(path.join(run,'expected-before-sql.json'),JSON.stringify(scrub(expectedSave),null,2));}});
 await a.click('手動保存今日早會');await until(()=>receipt.network.some(r=>r.caseId===currentCase&&r.rpc===patchRpc&&r.finished),'original save SQL ACK');await until(()=>a.eval("[...document.querySelectorAll('button')].some(n=>n.innerText==='手動保存今日早會'&&!n.disabled)"),'original save complete');
 qa.setRecordFault(null);assert.ok(expectedSave);const saved=await read();const {assertReportSaveResult}=await import('./report-history-save-oracle.mjs');assertReportSaveResult(saveBefore,saved,expectedSave,saveStarted);
 const physicalAfter=await physical();assert.deepEqual(physicalAfter.filter(r=>physicalBefore.some(b=>b.collection===r.collection&&b.entity_id===r.entity_id)),physicalBefore,'all original rows retain exact value/version/xmin/ctid');fs.writeFileSync(path.join(run,'physical-before-after.json'),JSON.stringify(scrub({before:physicalBefore,after:physicalAfter}),null,2));
 const mutations=[['snapshot-task-history',x=>x.agendaReports[0].snapshot.tasks[0].statusLogs.pop()],['snapshot-case-history',x=>x.agendaReports[0].snapshot.internalControlCases[0].statusLogs.pop()],['snapshot-meeting-history',x=>x.agendaReports[0].snapshot.meetings[0].statusLogs.pop()],['snapshot-member-history',x=>x.agendaReports[0].snapshot.tasks.find(t=>t.vesselProgress.length).vesselProgress[0].statusLogs.pop()],['audit',x=>x.auditLogs[0].action='WRONG'],['notification',x=>x.notifications[0].userId='WRONG'],['original-report',x=>x.agendaReports[1].title='WRONG'],['unselected-vessel',x=>x.vessels[1].name='WRONG']];for(const [id,mutate] of mutations){const bad=structuredClone(saved);mutate(bad.payload);assert.throws(()=>assertReportSaveResult(saveBefore,bad,expectedSave,saveStarted),assert.AssertionError,id);}receipt.oracleMutations=mutations.map(([id])=>id);
