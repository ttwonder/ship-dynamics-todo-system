...
Existing non-owner accounts preserve the pre-cutover contract: a valid legacy hash remains usable through `legacy-password`, while an originally blank Operator/Vessel credential remains `passwordless`. Owner remains native Supabase password mode. When the sole Owner's native password differs from the current website password, the gated `owner-password-session` bootstrap may verify that submitted password against the exact linked Owner in authoritative `ship_dynamics_app_state`, update only that Auth user's password through the server-side Admin API, and return a real session. The path requires the signed site-gate token, active Owner membership, exact `legacy_user_id`, native login mode, and network plus identity rate limits; it never returns AppData, hashes, passwords, or service credentials. A passwordless account promoted to Admin fails closed into native mode and must receive a manager reset before login.
