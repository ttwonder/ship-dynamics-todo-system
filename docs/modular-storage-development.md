...
### 免登入船端單船 Excel 有界往返（QA-only）

- 基線 `b1fedd46b53e24a576f52d1124721798c2095233`；實際入口為 `ship-itinerary.html → src/ship-itinerary-main.tsx → ShipItineraryPortal`，不是 App 登入 gate／NormalizedApp 或 LocalDemo。沒有產品 RED，所有產品 UI、權限、業務語意、SQL、模板與設定 bytes 保留。本機 `245` 個 product paths 對基線無差異，並重核上一片 `180` 個 raw product SHA-256。
- 新增 scoped `ship-itinerary-excel-local-fixture.mjs`、`verify-ship-itinerary-excel-browser.mjs` 與 `verify-ship-itinerary-excel-transport.mjs`。共享 QA 只加預設 false 的 `shipExcel` 選項，開啟時接原七個 public RPC 的參數簽名、原 lost-ACK hook，交易內 `SET LOCAL ROLE anon` 執行既存 SQL；不添加／替換 SQL function 或 grants。default public RPC 仍 404，default record read 結果與資料完全不變，沒有讓匿名船端要求 record actor。
- **10 個穩定原 UI／真 SQL scenario IDs**：`ship-anonymous-latest-sql`、`ship-export-formal-only`、`ship-export-formal-plus-alternatives`、`ship-reject-wrong-vessel-preserve-draft`、`ship-reject-multi-sheet-preserve-draft`、`ship-manual-confirm-decline-zero-save`、`ship-import-draft-cancel-zero-save`、`ship-manual-save-held-ack`、`ship-lost-ack-same-operation-status`、`ship-new-document-readback-reexport`。tracer 的前七個、失敗 run 的九個不重複加算。沒有新的 mounted controlled-callback cases。
- 原匿名選單只列三艘有效船、不列 inactive sentinel；人工選第一船，真正 public SQL latest read。原兩個按鈕由 Chrome 真下載正式檔及正式＋備選檔，ExcelJS 與獨立 openpyxl/ZIP 讀取實檔；正式檔只有單船資料頁，all-plans 分「正式方案／備選方案1」，同一 vessel/revision metadata、veryHidden `_Itinerary_Meta`、L/U 多選、`UTC+5:45` 的 05:45 本地時間對 UTC 00:00、報告起算 anchor 均核對。正式不混備選／未選船；另核 worksheet-property XML 順序與公式保留，不冒稱 native Microsoft Excel 已開檔或公式重算。
- 修改真下載單船檔後用原 file input 匯入；只成待人工核對草稿、零業務保存，blank previousPort metadata 按原規則補目前首列。使用同一 valid draft，wrong-vessel metadata 與真 all-plans 多資料頁均由原 UI 拒絕、保留全部可編輯欄值、零 save。原上一港／報告時間已填妥；原三項完整性 confirm 真正出現並先拒絕一次，拒絕與「取消編輯」均零 document/history/operation 變更。取消可釋放已取得的本船 lease，不把 lease bookkeeping 誤算業務保存。
- 再匯入、人工確認保存：真 SQL commit 後扣住 ACK，原編輯器仍顯示「保存中…」，未提前 close／報成功；釋出才成功。第二次修改再保存則於 SQL commit 後丟掉 ACK、扣住原同 operation status request；SQL status 確認同一 operation 才成功／close。只第一船 revision `7→8→9`，每次一筆 history／operation、精確 request rows／alternatives／actor／target／revision 與 SQL readback 一致；舊 append-only rows 不變。
- 新 browser document 重新選船、真 public SQL load，再由原兩按鈕下載核最新內容／revision 9。備選與兩艘未選船的 value/xmin/ctid、非空 records／report、其他所有表保留；legacy appstate/revisions 起始空且保持空，這證明零新增／雙寫，不冒稱覆蓋非空 legacy fixture。畫面與流程未跨原匿名邊界；沒有岸端 UI 再驗收、全角色、全船、手機/PDF 或寄信。
- 分層回歸：新 scoped HTTP／SQL transport **4** cases；原 record-write **28**（21 SQL＋7 SupabaseJS→SQL）、ship model/source gate 通過。未變路徑復用上一片 Excel helper、record-read **16**（12 SQL＋4 adapter）有效 receipts；typecheck、production build、diff／精確 product boundary 通過，build 只有既有 >500 kB chunk 提示。source、transport、helper／SQL、原 UI 場景分層，不相加冒稱 E2E。
- 歷史失敗保留：`03-ship-full` 前九條 UI 通過，但重匯出 metadata 預期錯用 SQL 毫秒；原 `parseDocument → normalizeInstant` 明確輸出秒精度，只修 QA 預期，raw SQL receipt 對照仍精確。`08-independent-xlsx` 把明確空字串 metadata 預期成 None；`11-reader-cell-trace` 定點讀回七個實檔後只修 reader。兩者均非產品 RED，舊 logs 不覆寫；browser 類共 tracer 一輪＋完整兩輪，沒有 blind retry 或新 review。
- 收據根：`C:/Users/tuotu/AppData/Local/hermes/cache/ship-itinerary-excel-b1fedd46/`。最新真 UI 為 `05-ship-full-final.log`／`ship-itinerary-excel-hDK0jL/evidence.json`；真 downloads、七個 xlsx（含修改／反控檔）、SQL snapshots、operation/history readback、screenshots、分層結果、逐命令 exit/log、完整 full-index patch、raw-tree ZIP/extraction 與本機 commit 綁定收據同根。owned Chrome／HTTP 已停、隔離 profiles 已清；failed run evidence 保留。
- 可重跑：`QA_EVIDENCE_ROOT=<既存私有目錄> node scripts/verify-ship-itinerary-excel-browser.mjs`（`--tracer-only` 只跑零業務寫入七場景）；`QA_EVIDENCE_ROOT=<既存私有目錄> node scripts/verify-ship-itinerary-excel-transport.mjs`。此為 installed PGlite SQL／匿名 role 與原 UI 證據，**不是 hosted Supabase/PostgREST/Realtime、真多連線／效能或正式上線驗收**。未知結果只證同 browser/actor/operation 的既有 recovery，不新增跨登入暫存架構。必要驗證後僅本機獨立 commit，無 Push／merge／部署／遠端查詢／正式 SQL／真 cron／正式 config 或使用者 browser storage 更動。
