...
- 所有已設定、帳戶啟用中的預設代管人員都另起一行，以半形 `()` 呈現；該船代管已激活者在人名後加 `*`。例如第一行 `張三`、下一行 `(李四、王五*)`；僅有代管者不留空白首行。不改實際代管權限，也沒有虛構一對一職務代理關係。
- 船隊、船型、英文船名、年份、噸數這五欄資料不換行，依欄寬自動縮小字體。中文船名仍可換行。
- 所有分管部門欄內容置中、保留換行及一致字級，不依賴部門名稱判斷，也不對人名作單格縮字。Excel 按完整合併範圍保留換行高度，不因單行縮字欄較長而多留列高。
- 「船隊」「船型」欄內，相鄰且相同的文字縱向合併；船型僅在同一船隊內合併，不跨船隊邊界，也不串接非相鄰重複值。不改船舶順序，維持不換行及依欄寬縮字。
- 同一部門欄內，相鄰且整組直接分管／代理 **人員 ID、直接／代管身分與激活狀態完全相同**時合併；同名不同人、代理不同、非相鄰及空白格不合併。保留原船舶排序。
- PDF 使用 rowspan；Excel 為真正的合併儲存格。維持 A4 直向單頁；資料過多時僅整頁等比例縮放，不會因單一欄位人名較多而特別縮字。「人員分管」工作表仍保留原功能。

PDF 與 Excel「船舶分管」表格最下方各列出使用者提供的兩行備註，括號維持半形 `()`；Excel 備註採跨欄合併、靠左、自動換行，並納入單頁列印範圍。備註僅為報表文字，不改實際分管、代管或加油業務權限；「人員分管」工作表不受影響。

## 上線順序（正式操作仍需使用者執行）

這些分管表排版與顯示調整只改前端；已安裝且唯讀核對通過者不需再跑 SQL。以下僅供尚未安裝新欄位者使用：
