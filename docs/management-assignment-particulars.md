...
- 管理中心 → 船舶 → 選船，在「船舶資料」維護 **年分／噸數**，沿用原本「保存變更」與管理權限。
...
- PDF／Excel「船舶分管」末尾新增年分、噸數，督導相對欄寬由 30 減至 15。
- 已啟用代理以括號呈現，不再顯示「代理：」。沒有虛構一對一職務代理關係。
- 同一部門欄內，相鄰且整組直接分管／代理 **人員 ID 與身分完全相同**時合併；同名不同人、代理不同、非相鄰及空白格不合併。保留原船舶排序。
