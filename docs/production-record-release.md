...
**正式 SQL 尚未執行、尚未 Push／merge／部署。不要把本文件或本機 PASS 當成正式切換成功。** 原 App 本人試用已回報正常，本輪未重跑全站／手機／PDF。`src/`、`public/`、原入口、UI 和 Pages workflow 均未修改。
...
- 原有 58 張 app 表在短暫 DDL 維護交易內鎖定；只保存 count＋ctid/xmin 的小型驗證向量，不排序或匯出完整歷史 JSON。
- 安裝前後舊表實體列向量必須完全一致；任何例外整筆 rollback。演練另在安裝末端故意報錯，確認新表／schema 一併回滾、舊資料未變。
...
- **首次增量／交付檔演練**：`verify-production-release-native.mjs` 10 個案例 PASS。實際執行 05、06、07–14；首次前先以 anon 進行一次新的 legacy 保存，確認承接的是較新版本，而非 fixture 初始版本。實際 anon member 保存／receipt PASS，再逆向轉回並確認該新修改保留。原正式 Itinerary、備選與已存報告歷史逐值保持。
...
- 任一步 ERROR、timeout、未知 ACK：**停在當前狀態**，不改 request ID、不刷新清草稿、不盲 Run 後一步。12 用於安裝已確認後的 operator state 讀回；若安裝尚未成立，用 06／catalog 而非呼叫不存在的 controls。
