...
- 初次讀取先取得同一工作區的來源，再依來源／epoch 讀取持久共同基線及版本下限；兩次來源核對之間才讀取業務資料。未完成核對，不發布來源、資料或改寫本機歷程。
- 原 `reload` 模式：沿前述 A/B 真提交與切換後明確重開原網頁，核对無業務寫入、設定／登入保留、B 在原編輯器可見；下一次 C 手動保存仍走目標入口，並核完整預期圖。
- `node scripts/verify-browser-authority-bootstrap.mjs` 執行從目前 App AST 取出的 bootstrap、讀取及安全同步函式。讀取與保存佇列由測試控制，驗證來源錯誤、設定途中改變、取消、異來源基線、草稿留存及防倒退；這不是原生資料庫或完整瀏覽器測試。
- 安全同步沿同一來源的版本下限，不拿舊來源的較大 revision 誤擋新來源。共同基線高於獨立儲存的下限時，較高值僅在最後來源／設定 fence 通過後保留於同來源記憶體下限；即使 rollback 清空合併基線，後續安全同步仍拒絕倒退，且不提前改寫儲存。來源尚未確認時拒絕一般讀取，不降級舊入口；本片不提供讀取失敗後免重開的自動重試。
- 沒有改動 JSX、登入／角色、原合併規則或自動上傳未知草稿。受控同步測試的佇列送出不是資料庫提交證据；原生 ACK 證據以獨立 `reload` 情境為準。

## 仍在後續整合範圍內

- 原網頁自然產生的 dirty cache／未知操作跨 reload 的完整恢復與真正保存；第二片的受控分支測試不代替這一列。
