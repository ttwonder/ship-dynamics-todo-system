...
- 船端頁面說明用途：以不對外的異常、暫時無法解決且需公司協助的高風險事項、物料／備件跟催、船上提議或對公司的建議為主。正常異常應依 DMP-FM01 流程；已報 DMP-FM01 不需重複提報內控，報內控不代表一律免報 DMP-FM01。不確定時可先報內控，由督導判斷是否進入正常 DMP-FM01 流程。
- 主頁及填寫視窗均提示：**提交成功後，船端無法修改；需更正時重新提交修正版，多餘項目由辦公室刪除。** 本次不新增船端讀取、修改或刪除已提交案件的權限。
- 船端表單新增本批共用、必填的「報告人姓名＋職務」（最多 120 字），例如 `王小明／大副`；單筆及批量每筆均自動在事項內文末尾附上 `報告人姓名＋職務：…`。岸端原表單不增加此欄，舊案件不回填；這是自填資訊，不是帳戶認證。
...
9. 報告人與未提交草稿一起保存；新待確認封包使用本機版本 v2，凍結已合併的完整內文。既有 v1 待確認封包則按舊規則原樣恢復，不補寫報告人、不更換操作編號或回執內容。尚未提交的舊草稿保留，下一次新提交須填寫報告人。

每筆事項（含自動附加報告人）／狀態各限 10,000 字；伺服器整批 JSON 上限 2,000,000 bytes，前端預留 JSON 編碼空間。原 UI 必填與設備故障細項規則保留。報告人併入既有內文字串，不新增 RPC 參數、資料庫欄位或 SQL migration。
...
報告人增量驗證涵蓋必填且缺漏時零新增、單筆／批次各筆完整內文、草稿刷新保存姓名職務、未知 ACK 後原樣恢復。v1 相容性使用固定前版實際序列化函式（`scripts/fixtures/ship-internal-control-v1.ts`）產生待確認內容，先在本機 SQL 完成原提交，再由新版原介面讀回該回執，核對沒有第二次新增；不是宣稱由舊版完整 UI 產生的自然升級測試。
