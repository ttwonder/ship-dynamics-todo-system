...
- `QA_EVIDENCE_ROOT=<repo 外的證據目錄> npm run test:itinerary-vessel-history-browser`：11 組原始 App 操作／幾何檢查；隔離 Chrome、測試資料、本機 PGlite，禁止外部網路，直接量測桌面兩行分組、欄位順序及下方行列不變。
...
兩行排版調整僅變更前端欄位順序、簡短欄名與樣式，不新增或修改 SQL；既有摘要 SQL 不需重跑。
